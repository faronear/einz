...
> `PROTOCOL.md`（通道在册/撤销）、`PROTOCOL_MULTIVERSE.md`（空间隔离）、`aimemo/productLens.zhcn.md` §3 §4.4 §12
...
| 对手 | 能力 | 我们的应对（见 §2） |
| --- | --- | --- |
| 服务器 / 运维者 | 读取数据库中一切、观察流量 | 端到端加密：只有密文与元数据（收发通道、时间、长度） |
| 网络窃听者 | 抓包、篡改 | TLS + AEAD（密文带完整性校验，篡改即解不开） |
| 陌生第三方拿到通道 | 持有物理设备 | 撤销该通道 + 通道上线自毁 + 系统锁 + 可选 App PIN（§4.1） |
| 通道上的恶意软件 | **仅在已越狱/root 通道上**可读取 App 私有目录/密钥 | 政策禁止 + 事件处置（§4.2、§5） |
...
| 场景 | 为什么 |
| --- | --- |
| 已解锁通道被物理取证 | 通道已解密的数据对持有者可见（productLens §3.1） |
| **已泄露的历史密文 + 对应旧密钥** | 密码学上不可追回——轮换也只能保护"未来"，见 §3 |
| 伴侣的**合法在网通道**被攻破 | 攻击者持有合法通道，能收到新消息（唯一防线是用户的通道卫生） |
...
| 控制 | 实现位置 | 说明 |
| --- | --- | --- |
| 消息端到端加密 | `shared/lib/src/crypto/message_crypto.dart` | Space Key → 每消息派生密钥；AEAD AAD 绑定 space/message/entrance/type/key_version |
| 通道在册状态 | `server/src/config.ts` `getEntranceStatus`（三态）+ `guard.requireSession` | 未登记（403 `FORBIDDEN`）或已撤销（403 `ENTRANCE_REVOKED`）通道**取不到密文**；会话还必须绑定 space（无 space 会话直接 401）。**两种 code 必须分开**：混淆会让"服务端库被清空"被客户端误判为撤销 |
...
| 口令托管（接入凭证） | `shared/lib/src/crypto/key_escrow.dart` | Argon2id + XChaCha20-Poly1305；**取包免通道认证**（见 §3.2） |
...
| 空间隔离 | `guard.ts` `entranceScopeClause` + `attachments.ts` | `GET /space`、`GET /entrances` 只返回本空间成员通道；附件读写校验 `attachments.space_id` 与会话一致（2026-09-15 评审 C2 修复）。legacy 无 space 会话只看到"不属于任何空间"的通道——**不回落成全局**，否则等于留后门 |
...
| 口令重设通知 | `server/src/escrow.ts` + WS `passphrase.rotated` | 改口令后其余通道收通知 / 离线补查 `updated_at` |
| 客户端不缓存共享口令 | `app/lib/data/app_lock.dart`（`AppLockPayload` 无口令字段） | 2026-09-14 起锁包与明文 payload 均不含口令——本地秘密面只剩 Space Key + 通道私钥；代价见 §4.7、§6 |
...
| 场景 | 轮换有用吗 | 原因 |
| --- | --- | --- |
| 通道丢失/被偷（会上线） | 不必需 | 通道在册状态已阻断取密文；通道上线即自毁（§2） |
| 通道丢失/被偷（永不上线，数据已被复制） | 只对"未来"有用 | 已复制的历史永远救不回 |
| 通道被越狱/root，密钥可能被复制而通道仍在用 | **最有用** | 没有"通道"可撤销——要作废的是**密钥材料**，见 §4.2 |
| 备份/整机镜像泄露 | 有用 | 与上同；`..._this_device` 已挡住"随备份迁移"这一半 |
| 服务器被攻破 | **零收益** | 服务器只见密文，本来解不开 |
| 想让被撤销通道失去历史 | **零收益** | 旧密钥 + 旧密文已出库，密码学上不可逆 |
| 伴侣通道被攻破 | 零收益 | 攻击者持在网合法通道 |
| 定期卫生 / 合规 | 低 | 两人空间，无合规要求 |

架构上还有一个不对称，决定了优先级：

- **取密钥**比**取密文**容易——密文必须持在册通道的会话 token（`/sync` 403），密钥只要知道口令
  （`key-escrow` 取包**免通道认证**）。
- 所以"拿到密钥但没有密文 = 没用"，而**撤销（掐密文链路）**才是主力防线、覆盖面最大；
  轮换是"密文也可能从别处泄露"时的**二阶防御**，防的是复合失败。

而它的成本在**分发**上（不是密码学）：新密钥要不被被撤销通道拿到地送到剩余通道，需要
通道公钥链路 + 可靠投递 + 归档缺口补偿 + 轮换协调，任何一环断了都是"消息永久解不开"。
...
- 代价：伴侣通道需重新接入一次（邀请链接 + 共享口令）；历史留在旧空间（可保留只读）。

对两人私聊而言，这在"怀疑密钥泄露"这种低频事件上比造轮换机制划算得多。

### 3.5 若将来要恢复轮换（前置条件，缺一不可）

1. **分发链路**：`GET /space` 暴露通道公钥（当前 `SpaceEntrance` 无此字段）+ 密封密钥的
   投递方式（可用新消息类型承载 sealed box，服务端零改动）；
2. **归档随密保箱分发**：`aimemo/escrowArchivedKeys.md`（现为 `[搁置]`）——否则新通道接入后旧消息解不开；
3. **口令同步轮换 + 线下告知伴侣**：否则被撤销通道凭口令仍可取到新密钥（§3.1 不对称）；
4. **缺口补偿与协调**：离线通道错过轮换、多通道避免重复轮换；
...
> 原则：**先掐密文链路（撤销），再谈密钥材料**；历史密文不可追回是前提，不要承诺做不到的事。

### 4.1 通道丢失 / 被偷

1. 在**同空间的另一条通道**上撤销它（2026-09-16：同 space 内可互撤，A 没有第二条通道时
   伴侣 B 也能替他撤；每次都要验口令，因为撤销会让对方自毁本地数据）：
   - TUI：`/entrances` 看序号 → `/revoke <序号>` → **抄一遍目标通道名**确认（2026-09-26：
     原先是输 `yes`——固定词闭眼就能敲过去，换成抄名字逼人看清撤的是哪一台）→ 输入共享口令；
   - 运维/无客户端可用时：`curl -X POST .../entrances/<id>/revoke -d '{"passphrase":"…"}'`（`DEPLOYMENT.md` §5.3）；
   - App 里的通道列表入口**尚未做**（届时同样要口令 + 二次确认）。
2. 该通道：无法再认证/同步/发送；若它上线，App 会**自我销毁**本地数据（§2）。
...
2. 检查口令强度：取包免通道认证，弱口令可被离线爆破 → 若口令弱，**立即改口令**（旧口令作废，见 `KEY_ESCROW.md` §7）；
3. 检查 `/entrances` 有无陌生通道（同空间可见）；
4. 若服务器数据库里的密文 + 你的口令都泄露 → 按 §4.2。

### 4.4 共享口令泄露或怀疑

1. 在任一通道上**修改口令**（App 聊天页 ⋯ / TUI `/passphrase`）：服务端密保箱重加密，旧口令立即失效，其余通道收 `passphrase.rotated` 通知；
   - ⚠️ **改完必须线下把新口令告知伴侣**：新口令不在网络上传递，而且**客户端自 2026-09-14 起
     不本地缓存口令**（§2）——口令的唯一载体只有两人各自的记忆 + 被它加密的服务端密保箱。
     不告知的话，对方日后接入新通道或做恢复会失败。这是改口令的唯一真实摩擦点；
2. 注意：改口令**不换密钥**（箱子里还是同一把 Space Key）→ 已经取过箱子的人不受影响；若同时怀疑密钥，按 §4.2。

### 4.5 伴侣通道可疑

无技术手段（合法在网通道）。→ 线下沟通；必要时撤销该通道 + 重建空间。
...
**影响：** 密保箱是"凭口令接入新通道"的**唯一**凭据来源。丢失后新通道无法凭口令接入、
忘记 PIN 的通道无法恢复——直至重建。**既有在网通道正常聊天不受影响**（Space Key 都在本地）。

**处置（重建密保箱，任一即可）：**

1. **在任一已解锁通道上走"修改口令"**（App 聊天页 ⋯ / TUI `/passphrase`）——客户端检出
...
**为何跳过旧口令是安全的：** 执行重建的通道已通过通道认证、且已持有 Space Key——重建只是
把"同一把 Space Key 换一个口令再加密上传"，不新增任何存取权限。代价见 §6。

**兜底：** 若某通道的 PIN 已忘记**且**密保箱又丢失，该通道的本地 Space Key 无法解出——只能
...
2. **通道**：不 root、不越狱；开启系统锁屏与整机加密；给 App 设 PIN（跳过 PIN 会丢失一层防线）。
3. **恢复码**：备份导出的 12 词恢复码**离线**保存（纸/离线介质），丢失即无法恢复归档。
4. **通道清单**：定期检查在网通道（App 通道列表），撤销不再使用的。
...
| 风险 | 状态 | 备注 |
| --- | --- | --- |
| 已被对方/被撤销通道同步的密文 + 旧密钥 | **接受** | 密码学不可逆；轮换也不解决（§3.1） |
| 越狱/root 通道上的密钥可被提取 | **接受** | 政策禁止 + §4.2 处置 |
| 伴侣合法通道被攻破 | **接受** | 无技术手段 |
...
| `GET /avatar/:memberId` 免认证可读 | **接受** | v2 的 memberId 是随机 UUID、不可枚举；头像是本人自愿上传的展示图，同空间成员本就该看到。legacy `memberA/memberB` 可枚举但只影响 v1 轨道（收敛计划见 D3）。加鉴权需改成带 token 取图 + 客户端缓存失效重做，收益不抵成本（2026-09-15 评审 C1） |
| 服务端密保箱为无冗余单点 | **接受** | 客户端不再缓存口令（§2）→ 不再有解锁自动重传；丢失后需手动重建（§4.7）。**无任何在网通道持有 Space Key 时**只能走 CLI 备份恢复 |
