...
> `PROTOCOL.md`（白名单/撤销）、`aimemo/productLens.zhcn.md` §3 §4.4 §12
...
| 陌生第三方拿到设备 | 持有物理设备 | 白名单撤销 + 设备上线自毁 + 系统锁 + 可选 App PIN（§4.1） |
...
| 服务器白名单 | `server/src/config.ts` `isActiveDevice` + `messages.ts:101`（`/sync` 403） | 未登记/已撤销设备**取不到密文** |
| 设备撤销 | `server/src/devices.ts` `revokeDevice`（移白名单 + 清 Push + 清会话） | 被撤销设备无法认证/同步/发送 |
...
| 场景 | 轮换有用吗 | 原因 |
| --- | --- | --- |
| 设备丢失/被偷（会上线） | 不必需 | 白名单已阻断取密文；设备上线即自毁（§2） |
...
- **取密钥**比**取密文**容易——密文必须持白名单设备 token（`/sync` 403），密钥只要知道口令
  （`key-escrow` 取包**免设备认证**）。
...
- 代价：伴侣设备需重新接入一次（邀请码 + 密保口令）；历史留在旧空间（可保留只读）。
...
1. 在另一台设备上**撤销**该设备（App：设备列表；CLI：`DELETE /devices/<id>`）。
