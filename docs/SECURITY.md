...
1. 在**同空间的另一台设备**上撤销它：`POST /devices/<id>/revoke`，请求体带**空间密保口令**
   （2026-09-16：同 space 内可互撤，A 没有第二台设备时伴侣 B 也能替他撤；每次都要验口令，
   因为撤销会让对方自毁本地数据）。**客户端入口（App 设备列表 / CLI 命令）尚未做**——
   当前只能由运维用上面的 curl 执行（`DEPLOYMENT.md` §5.3）。
