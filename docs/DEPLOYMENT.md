...
App 端同理：设置页选「创建秘境」或「加入秘境」，扫开通码二维码 / 粘贴邀请链接，再输共享口令。
...
- App：对话页菜单 → 高级 → **销毁本通道**（空间级：`app/lib/widgets/reset_entrance.dart`
  → `removeSpace` 只清**这个空间**——消息/附件/回执/锚点/草稿/`spaces` 行/profile 与设置键
  + 留存明文与媒体缓存；**不动本机上的其它空间**）。
- TUI：`/reset`（= 删掉这个 store 文件）或 `rm -f ~/.einz/*.json`；再清附件缓存目录。
- 或直接卸载重装。安装级 `install_uid` 存在 `app_state` 里，**整机清空 / 卸载重装**时整表
  清掉 → 下次调用惰性生成 = 轮换（App 里销毁单个通道不动它）。
