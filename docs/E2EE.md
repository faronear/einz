...
系统提供**两条并列的 Space Key 分发路径**，按场景选用；app 默认走 escrow，seal 为高级/CLI 路径。

**方案一：seal 密封交换（§7.1-7.2，CLI `config`/`import`、app「高级导入」）**
...
seal 密封交换在双方公钥互知后才可用，保留为 CLI 与「高级导入」选项。
