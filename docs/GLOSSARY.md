...
1. **界面用词（2026-09-22 晚定，2026-09-23：「入口」→「通道」）。** 三个词各管一段，**不再靠加限定词消歧**：
   ...
   - **「开通码」= 一次性进入授权**（英文仍 `token`，2026-09-23 起；取代「令牌」，更早是「邀请码」）。
     ...
     **为什么再从「令牌」改「开通码」（2026-09-23）**：①「令牌」偏技术黑话，
      ...
      **2026-09-24 补齐服务端**：原先只有服务端没跟上——`server/src/app.ts` 的邀请落地页写的是
      「邀请码」，与 App/TUI 的「开通码」不一致；已统一为「开通码」（连带代码注释与
      `server/test` 里指代该名词处）。至此四面（App / TUI / 服务端 / 文档）一致。
      剩下的「邀请」见下一条——那些确实是在**对人**说话。
