...
状态：**2026-09-22 定稿**（老板）。本文是**唯一**的分层定义，`docs/DATABASE.md`、
...
| **登记项（入口）** | 某安装在某秘境里的一次登记：一套密钥对 + 一个身份 + 一个会话锚点 | `devices` 表的**一行**；`device_id` | 「设备」（跨秘境会歧义处加限定词：本机名称 / 本秘境里的名称·公钥） |
...
1. **界面用词服从用户习惯。** 界面里继续用「设备」表示"这个秘境里的一台机器"——
   用户的说法就是"我的手机/电脑"。只在**跨秘境会产生歧义**的地方加限定词
   （例如「本机信息（本秘境）」「本秘境里的名称/公钥」），不引入「入口/关口」
   这类抽象名词：它们会与 UI 里既有的「秘境向导/秘境入口」以及代码与文档里的
   「闸门」（破坏性操作的确认闸门）撞车。
...
**有利条件**：`device_id` 几乎只出现在**响应**里——客户端主动发送它的地方只有
`/devices/:id/revoke` 的路径参数。所以"双名并存"很好做：服务端同时输出
`device_id` 与 `entry_id`，老客户端继续能用，新客户端读新名、回退旧名，
**不需要客户端先升级也不会坏**。
