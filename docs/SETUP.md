...
| 设备 A 密钥信封 | `sealed_space_key_a`（仅 A 可解） | 一次性流转，导入 A |
| 设备 B 密钥信封 | `sealed_space_key_b`（仅 B 可解） | 一次性流转，导入 B |
...
5. 输出：config.json + 密钥信封 A + 密钥信封 B
```

- 密钥信封用公钥 A/B 分别加密：**只有对应设备能解开**（E2EE.md §7.1）。
...
- 设备 A 导入自己的密钥信封 → 用本机私钥 `crypto_box_seal_open` 解出 Space Key → 存入安全存储。
- 设备 B 同理。
- 导入后删除密钥信封临时文件（避免再次流转）。
