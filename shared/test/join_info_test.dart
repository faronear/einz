// JoinInfo 单测：A → B 加入信息的编解码（onlyspace-join-v1 格式）。
...
void main() {
  test('encode/decode 往返：普通口令', () {
    ...
    expect(raw, 'onlyspace-join-v1?space=space-demo&p=123456');
  ...
  test('格式不符返回 null', () {
    ...
    expect(JoinInfo.decode('onlyspace-join-v1?space=only'), isNull); // 缺 p
    expect(JoinInfo.decode('onlyspace-join-v1?p=123'), isNull); // 缺 space
