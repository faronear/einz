...
///
/// 格式：`einz-join-v1?space=<spaceId>&p=<passphrase>`（旧 onlyspace-join-v1 链接兼容解析）
///
...
class JoinInfo {
  ...
  /// 兼容旧链接（改名前的 onlyspace-join-v1）。
  static const String legacyPrefix = 'onlyspace-join-v1';
  ...
  /// 解析分享字符串；格式不符或参数缺失返回 null（兼容新旧两种前缀）。
  static JoinInfo? decode(String raw) {
    final matched = raw.startsWith('$prefix?')
        ? prefix
        : (raw.startsWith('$legacyPrefix?') ? legacyPrefix : null);
    if (matched == null) return null;
    try {
      final params = Uri.splitQueryString(raw.substring(matched.length + 1));
