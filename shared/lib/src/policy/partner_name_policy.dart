/// 用户名称（partner 显示名）规则（**唯一来源**，App / TUI / CLI 共用；服务端
/// `server/src/partnerName.ts` 是同一约定的 TS 版本，改动请两边同步）。
///
/// 老板 2026-09-16 定：最多 32 字符，只允许 **中文字、英文字母、数字、`_`、
/// `-`、表情符 emoji**。
///
/// 与设备名（entrance_name_policy.dart）的差别：**这里允许 emoji**——名字是给人看的
...
const int kPartnerNameMaxLength = 32;
...
/// （❤️）、`\u200D` ZWJ（👨‍👩‍👧 家庭组合）、`\u20E3` 键帽（1️⃣）。
final RegExp kPartnerNameAllowedChar = RegExp(
...
final RegExp kPartnerNamePattern = RegExp(
  r'^[0-9A-Za-z_\-\p{Script=Han}'
  r'\p{Extended_Pictographic}'
  r'\u{1F1E6}-\u{1F1FF}\u{FE0F}\u{FE0E}\u{200D}\u{20E3}]+$',
  unicode: true,
...
enum PartnerNameViolation {
  /// 空（或 trim 后为空）。
  empty,

  /// 超过 [kPartnerNameMaxLength] 个码点。
  tooLong,
...
/// 校验用户名称：返回 null = 通过。
PartnerNameViolation? checkPartnerNamePolicy(String name) {
  final value = name.trim();
  if (value.isEmpty) return PartnerNameViolation.empty;
  if (!kPartnerNamePattern.hasMatch(value)) {
    return PartnerNameViolation.illegalCharacter;
  }
