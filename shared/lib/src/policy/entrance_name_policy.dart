/// 通道名规则（**唯一来源**，App / TUI / CLI 共用；服务端 `server/src/entranceName.ts`
/// 是同一约定的 TS 版本，改动请两边同步）。
///
/// 老板 2026-09-16 定：通道名只允许 **中文字、英文字母、数字 0-9、下划线 `_`、
/// 中划线 `-`**，最长 32 字符。
///
/// 为什么收字符集：通道名会出现在顶部条、通道列表、消息落款与推送提示里，是一路
/// 被拼接显示的短标识；空格/emoji/标点会让"名字边界"变得模糊（哪儿是名字、哪儿是
/// 分隔），也会在日志与终端渲染里制造歧义。统一收口到这个很小的集合，展示层不需要
/// 再为怪字符做转义。
library;

/// 最长字符数（与 TUI 此前对宿主名的截断一致；顶部条/通道列表放得下）。
...
/// 校验用户输入的通道名：返回 null = 通过。
///
/// 用户输入（TUI `/entrance <名字>`、App 改名弹窗）走这里——不合规就拒绝并提示重输，
