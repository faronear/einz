...
/** GET /entrances：设备列表（含 partner 映射）。
 *  注意：不在本接口刷新调用方 last_seen——last_seen 只由 WS 连接/心跳/断开维护，
 *  否则任何轮询客户端都会让自己"永远新鲜"（对方误判在线，见 chat_page 在线判定）。
 *  范围：**仅本会话可见的设备**（该空间成员；见 guard.entranceScopeClause）——
 *  此前直出全局 entrances 表，跨空间泄漏 partner/公钥/在线状态（2026-09-15 评审 C2）。
...
export function listEntrances (
  ...
  const rows = getDb()
    .prepare(
      `SELECT d.entrance_id, d.partner_id, d.status, d.last_seen, d.entrance_name
         FROM entrances d
        WHERE ${scope.sql}
...
/** POST /entrances/:id/revoke：撤销**本空间内**的另一台设备
 *  （白名单移除 + 清 Push Token + 清会话，PROTOCOL.md §7.2）。
 *
 * 授权规则（老板 2026-09-16 定稿）：
 * 1. **同 space 内可互撤**——不限于"同一 partner 的另一台设备"：A 的手机丢了、A 又没有
 *    第二台设备时，伴侣 B 也能替他撤掉那台（此前的实现是"任何在册设备能撤任何设备"，
 *    连空间都不校验；而文档写的是"仅限同 partner"，代码比文档更宽）；
...
export async function revokeEntrance (
  ...
  // 目标设备必须属于**本会话所在空间**（entrances 表没有 space 列，空间归属走
  // entrances.partner_id → space_members）
  if (target.partner_id.length === 0 || !isSpaceMember(caller.space_id, target.partner_id)) {
 ...
 * - connection_events / entrance_activity 是**只追加审计表**，刻意保留——"这台设备来过"
...
/**
 * POST /entrances/uid：补登本设备的安装级标识（多空间）。
 *
 * 存在的理由：`install_uid` 是随 create/join 一起上报的，但**存量设备**（多空间
...
/**
 * POST /entrances/name：更新本设备名称（已登记设备 TUI 里改名后同步到后台，显示层用）。
...
/**
 * POST /partners/name：更新本设备的 partner 显示名（/rename 命令，显示层用）。
 * - 认证：session token（bearer）；仅 active 设备可改自己 partner 的名称。
 */
export function updatePartnerName (
  ...
  if (row.partner_id) {
    getDb()
      .prepare(`UPDATE space_members SET display_name = ? WHERE space_id = ? AND partner_id = ?`)
      .run(partnerName, space_id, row.partner_id)
  }
  console.log(
    `[einz] 更新 partner 名称: partner=${row.partner_id}（${partnerName}）`
  )
