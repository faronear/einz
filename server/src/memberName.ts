...
export function assertMemberName(name: string): void {
  ...
  if ([...value].length > MEMBER_NAME_MAX_LENGTH) {
    throw new ApiError(
      "INVALID_REQUEST",
      `name 最长 ${MEMBER_NAME_MAX_LENGTH} 个字符`,
      400
    );
  }
  if (!MEMBER_NAME_RE.test(value)) {
    throw new ApiError(
      "INVALID_REQUEST",
 ...
 *  `shared/lib/src/policy/member_name_policy.dart`，改动务必两边同步。 */
export function isSameMemberName(a: string, b: string): boolean {
