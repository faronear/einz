...
/**
 * 安装级标识（`entrances.install_uid`）：客户端生成、随 create/join 上报，
 * 用来把**同一台物理设备在不同空间里的多个 entrance_id** 关联起来。
 *
 * 定位（老板 2026-09-22 定）：
 * - 只做**服务端侧认知**（运维/审计/将来"整机退役"用），不承担任何客户端功能，
 *   **不参与任何破坏性操作的授权或范围判断**——那是本机独占因子（通道名 + 锁屏码）的事。
 * - 生命周期 = **安装级**：随该安装的本地数据整表清除而轮换（卸载重装、整机清空）。
 * - **绝不进任何响应体**：`/space`、`/entrances`、WS 广播都不带，成员之间互不可见。
 *
 * 唯一的**否决**用途（2026-09-23 老板追加：前后端一致）：
 * `joinSpace` 用它拒绝"同一台设备在同一个空间里重复加入"（`ENTRANCE_ALREADY_EXISTS`）。
 * 这一条不与上面的定位冲突——它是 denylist（只拒绝，不授权、不扩大任何操作的范围、
 * 也不决定任何破坏性操作作用于谁）；而且它只是客户端闸门的第二道，客户端才是主拦点。
 * 除此之外，**不要**再拿 install_uid 去授权或界定任何操作。
 *
...
/** 显式补登（POST /entrances/install-uid）用：非法直接 400，让客户端的问题暴露出来。 */
