...
/** 用户名称（person 显示名）规则（老板 2026-09-16 定）：最多 32 字符，只允许
 *  **中文字、英文字母、数字、`_` `-`、表情符 emoji**。
 *
 *  与设备名的差别是**允许 emoji**——名字是给人看的亲昵称呼（"小猪🐷"），设备名
 *  是给机器看的短标识，所以两条规则不合并（见 deviceName.ts）。
 *
 *  客户端同款规则在 `shared/lib/src/policy/person_name_policy.dart`（Dart/TS 无法
 *  共用一份代码，改动务必两边同步）。
 *
...
export function assertPersonName(name: string): void {
  ...
  if ([...value].length > PERSON_NAME_MAX_LENGTH) {
    throw new ApiError(
      "INVALID_REQUEST",
      `name 最长 ${PERSON_NAME_MAX_LENGTH} 个字符`,
      400
    );
  }
