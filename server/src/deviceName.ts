...
/** 设备名规则（老板 2026-09-16 定）：只允许 **中文字、英文字母、数字 0-9、
 *  下划线 `_`、中划线 `-`**，最长 32 字符。
 *
 *  与客户端 `shared/lib/src/policy/device_name_policy.dart` 是同一约定的两份实现
 *  （Dart/TS 无法共用一份代码），改动务必两边同步。
 *
 *  服务端为什么要自己再查一遍：设备名是**别处传来的数据**（create/join 的
 *  device_name、POST /devices/name），前端校验只是体验，服务端这道才是约束——
 *  老版本客户端、直接打接口的脚本都得被收口。
 *
 *  两条不同的处理，别混用：
 *  - [normalizeDeviceName]：create/join 带的名字（多数是客户端自动取的宿主机名/
 *    设备型号，用户没表达过意愿）→ 不合规字符换成 `_`；
 *  - [assertDeviceName]：显式改名（用户主动输入的）→ 不合规直接 400，让客户端
 *    提示重输，而不是悄悄把人的输入改掉。 */
...
export function assertDeviceName(name: string): void {
  if (typeof name !== "string") {
    throw new ApiError("INVALID_REQUEST", "device_name 必须是字符串", 400);
  }
  ...
  if ([...value].length > DEVICE_NAME_MAX_LENGTH) {
    throw new ApiError(
      "INVALID_REQUEST",
      `device_name 最长 ${DEVICE_NAME_MAX_LENGTH} 个字符`,
      400
    );
  }
  if (!DEVICE_NAME_RE.test(value)) {
    throw new ApiError(
      "INVALID_REQUEST",
      "device_name 只能包含中文字、英文字母、数字、下划线(_)、中划线(-)",
      400
    );
  }
}
 ...
 *  （device_name 允许为空，展示层用 device_id 兜底）。 */
export function normalizeDeviceName(raw: unknown): string | null {
  ...
  for (const ch of source) {
    out += DEVICE_NAME_ALLOWED_CHAR.test(ch) ? ch : "_";
  }
  out = [...out].slice(0, DEVICE_NAME_MAX_LENGTH).join("");
