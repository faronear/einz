 ...
 */
export function requireSession(token: string | null): EntranceSession {
  if (token == null || token.length === 0) {
    throw new ApiError("UNAUTHORIZED", "missing bearer token", 401);
  }
  const { entrance_id, space_id } = resolveSession(token);
  const entranceStatus = getEntranceStatus(entrance_id);
