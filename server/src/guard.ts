...
/** 认证 + 白名单校验。token 为 null（未带凭证）→ 401；设备未登记/已撤销 → 403。 */
export function requireSession(cfg: ServerConfig, token: string | null): DeviceSession {
  ...
  return { device_id, space_id, person_id: getDevice(cfg, device_id)?.person_id ?? "" };
}
 ...
 *
 * - 有 space 的会话：只返回该空间在册成员名下的设备；
 * - 无 space 的 legacy 会话（v1 轨道）：只返回不属于任何空间的设备。
 *   刻意**不是**"回落成全部"——否则拿一个不带 space_id 的 legacy 会话就能重新
 *   看到全局设备表，等于给隔离开了后门。
 *
 * 调用方须给 devices 表起别名 `d`。
 */
export function deviceScopeClause(spaceId: string | null): { sql: string; params: string[] } {
  ...
  return {
    sql: `NOT EXISTS (SELECT 1 FROM space_members sm WHERE sm.person_id = d.person_id)`,
    params: [],
  };
}
...
export function requireSpaceMember(
  ...
  const sess = requireSession(cfg, token);
