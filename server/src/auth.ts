...
import { getEntrance, getEntranceStatus } from "./config.js";
...
export async function createChallenge(entranceId: string, spaceId: string): Promise<ChallengeResult> {
  const entranceStatus = getEntranceStatus(entranceId);
  if (entranceStatus === "revoked") {
    throw new ApiError("ENTRANCE_REVOKED", "entrance revoked", 403);
  }
  if (entranceStatus === "missing") {
    throw new ApiError("FORBIDDEN", "entrance not in whitelist", 403);
  }
  ...
  const entrance = getEntrance(entranceId)!;
  const challenge = await randomBytes(32);
  const challengeId = toB64(await randomBytes(16));
  const sealed = await sealFor(entrance.public_key, challenge);
  ...
  getDb()
    ...
    .run(challengeId, entranceId, spaceId, toB64(challenge), Date.now() + CHALLENGE_TTL_MS);
...
export function verifyChallenge(challengeId: string, plaintextB64: string): SessionResult {
  const row = getDb()
    ...
    .get(challengeId) as
    | { challenge_id: string; entrance_id: string; space_id: string | null; challenge: string; expires_at: number; used: number }
    | undefined;
  ...
  db.prepare(
    `INSERT INTO sessions (session_token, entrance_id, space_id, expires_at, created_at) VALUES (?, ?, ?, ?, ?)`
  ).run(hashSessionToken(sessionToken), row.entrance_id, sessionSpaceId, now + SESSION_TTL_MS, now);
...
export function resolveSession(token: string): { entrance_id: string; space_id: string | null } {
  const stored = hashSessionToken(token);
  const row = getDb()
    .prepare(`SELECT entrance_id, space_id, expires_at FROM sessions WHERE session_token = ?`)
    .get(stored) as { entrance_id: string; space_id: string | null; expires_at: number } | undefined;
  ...
  return { entrance_id: row.entrance_id, space_id: row.space_id };
}

export function touchLastSeen(entranceId: string): void {
