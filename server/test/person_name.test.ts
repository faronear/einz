/**
 * 回归：用户名称（person 显示名）规则（老板 2026-09-16 定）——最多 32 字符，
 * 只允许中文字 / 英文字母 / 数字 / `_` `-` / emoji。
 *
 * 与设备名（device_name.test.ts）的差别是**允许 emoji**：名字是给人看的亲昵称呼。
 * 名字一律由用户输入（没有自动取名这条路），所以服务端只做**拒绝**（400），
 * 不做消毒——静默改写人的名字等于名字莫名变了。
 *
 * 客户端同款规则在 shared/lib/src/policy/person_name_policy.dart（改动请同步）。
 *
 * 运行：npm test（tsx test/person_name.test.ts）
 */
...
import { assertPersonName } from '../src/personName.js'
...
test('端到端：create 的两个名字与改名都按白名单收口', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'einz-person-'))
  const port = freePort()
  ...
  try {
    ...
    assert.ok(Object.values(space.person_names).includes('小猪🐷'), 'emoji 名字应原样入库')

    // 3) 改名：不合规 400；合规（emoji）200 生效
    for (const bad of ['Mr Lukas', '名字。', 'a'.repeat(33)]) {
      const res = await req(port, '/devices/person-name', {
        method: 'POST',
        headers: auth,
        body: JSON.stringify({ person_name: bad })
      })
      assert.equal(res.status, 400, `不合规改名应被拒: ${bad}`)
    }
    const ok = await req(port, '/devices/person-name', {
      method: 'POST',
      headers: auth,
      body: JSON.stringify({ person_name: '阿猪🐷_01' })
    })
    assert.equal(ok.status, 200, '合规改名应成功')
    const space2 = (await (await req(port, '/space', { headers: auth })).json()) as {
      person_names: Record<string, string>
    }
    assert.ok(Object.values(space2.person_names).includes('阿猪🐷_01'), '改名应生效')
  } finally {
    proc.kill()
  }
})
