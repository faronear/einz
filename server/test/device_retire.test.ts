/**
 * 回归：本机自助退役 `POST /devices/retire`（2026-09-21 新增）。
 *
 * 客户端"重置设备"原先只清本地数据，服务端这台设备的注册表项、Push Token、会话全留着，
 * 对方 /devices 里是一台永远在线的幽灵——而 POST /devices/:id/revoke 禁止自撤，谁也删不掉它。
 *
 * 两条**必须立住**的边界（老板 2026-09-21 定稿）：
 * 1. 退役**不发 device.revoked**：那是客户端自毁本地数据的授权信号。退役只认 session，
 *    若由它发出这帧，偷到 session 的人就能远程擦设备，等于给口令闸门挖了条旁路。
 * 2. 退役要**立刻让对端看到下线**：光靠客户端自己断连不可靠（客户端可能迟迟不退），
 *    而心跳每 30s 还会给连接续 last_seen，把已退役的设备刷成在线。
 *
 * 运行：npm test（tsx test/device_retire.test.ts）
 */
...
import { listDevices, retireDevice } from '../src/devices.js'
...
function seed (): void {
  ...
  const dev = db.prepare(
    `INSERT INTO devices (device_id, person_id, public_key, status, last_seen, created_at)
     VALUES (?, ?, 'pk', 'active', ?, ?)`,
  )
...
test('retireDevice：清会话/Push/待签 challenge，设备置 revoked，且不波及其他设备', () => {
  const dir = mkdtempSync(join(tmpdir(), 'einz-retire-'))
  try {
    openDb(join(dir, 'retire.db'))
    seed()

    assert.equal(retireDevice('tok-a1').ok, true, '自助退役应成功')

    const row = getDb()
      .prepare(`SELECT status, last_seen FROM devices WHERE device_id = 'a1'`)
    ...
    // devices 行**保留**：删了它，messages.sender_device_id 会失去归属，且配置层会把它
    // 当成"未登记"（FORBIDDEN）而非"已退役"（DEVICE_REVOKED）——两者在客户端语义不同
    assert.equal(count(`SELECT COUNT(*) AS n FROM devices WHERE device_id = ?`, 'a1'), 1)
    ...
    for (const [table, why] of [
      ...
      assert.equal(count(`SELECT COUNT(*) AS n FROM ${table} WHERE device_id = ?`, 'a1'), 0, why)
    }

    // 对方的一切不受影响
    const peerRow = getDb()
      .prepare(`SELECT status FROM devices WHERE device_id = 'b1'`)
      .get() as { status: string }
    assert.equal(peerRow.status, 'active', '不得动对方设备')
    assert.equal(count(`SELECT COUNT(*) AS n FROM push_tokens WHERE device_id = ?`, 'b1'), 1)
    assert.equal(count(`SELECT COUNT(*) AS n FROM sessions WHERE device_id = ?`, 'b1'), 1)
    assert.equal(count(`SELECT COUNT(*) AS n FROM challenges WHERE device_id = ?`, 'b1'), 1)

    // 会话已删：同 token 再来一次 → 401（不是 403 DEVICE_REVOKED，那条属于"被人撤销"）
    let again: unknown
    try {
      retireDevice('tok-a1')
    } catch (e) {
...
test('retireDevice：无凭证 → 401（身份只认 session，没有口令兜底）', () => {
...
test('retireDevice：对端收到 peer.offline，**绝无** device.revoked；本机连接不被关', async () => {
  ...
  try {
    ...
    await waitFor('b1 收到 a1 的 peer.offline', () =>
      b1.frames.some(f => f.type === 'peer.offline' && f.payload.device_id === 'a1'))

    // 2) 自毁信号绝不由退役发出：否则"偷到 session"等同于"远程擦设备"
    await sleep(300)
    assert.deepEqual(
      [...a1.frames, ...b1.frames].filter(f => f.type === 'device.revoked'),
      [],
      'device.revoked 只属于"被对方口令撤销"这条路径，退役不得发出',
    )

    // 3) 本机的 WS 不被主动关闭：任何关闭都会让客户端重连撞 403 DEVICE_REVOKED → 同样自毁
    ...
    // 4) 已摘出在线表：心跳不再给它续 last_seen，/devices 里也不再显示在线
    await waitFor('a1 从在线表消失', () => {
      const row = listDevices('tok-b1').devices.find(d => d.device_id === 'a1') as
        | Record<string, unknown>
        | undefined
      return row?.connected_at == null
    })
    const caller = listDevices('tok-b1').devices.find(d => d.device_id === 'a1') as
      | Record<string, unknown>
      | undefined
    assert.equal(caller?.status, 'revoked', '对方 /devices 应看到 a1 已退役')
