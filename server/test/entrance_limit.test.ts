 ...
 * 额度**（devices 行只标记 revoked、不删除，否则"开通→销毁→再开通"可无限刷）；
...
import { retireDevice } from '../src/devices.js'
...
test('销毁不退额度：退役（revoked）后仍占额度，不能靠反复开通/销毁刷量', async () => {
  await withDb(async () => {
    ...
    const second = joinSpace(space.joinToken, 'pk-b', 'Pixel', 'female', 1)
    // 第二台设备自助退役：devices 行只标记 revoked、不删除
    retireDevice(second.sessionToken)
    const revoked = getDb()
      .prepare(`SELECT status FROM devices WHERE device_id = ?`)
      .get(second.deviceId) as { status: string }
    assert.equal(revoked.status, 'revoked')
    assert.equal(entranceCount(space.spaceId), 2, '撤销的通道仍计入额度')

    const third = createJoinToken(space.spaceId)
    assert.throws(
      () => joinSpace(third.joinToken, 'pk-c', 'Mac', 'male', 1),
...
test('上限按空间隔离：A 满了不影响 B', async () => {
  await withDb(async () => {
    ...
    joinSpace(a.joinToken, 'pk-a2', 'Pixel', 'female', 1)
    const b = await createSpace(
      undefined, '我', 'male', '伴侣', 'female',
      undefined, undefined, 'pk-b1', 'Mac',
    )
    // B 只登记了创建者这一条 → 仍能加入
    joinSpace(b.joinToken, 'pk-b2', 'iPad', 'female', 1)
