 ...
 * 四个必须立住的点：
 * 1. 只数"晚于我读取水位"的；
 * 2. **必须走 person 维度**排除自己：同一身份的第二台设备发来的消息不是未读
 *    （只比 device_id 会把它算进来——这是最容易错的点）；
...
test('unread：只数"晚于我读取水位"的对方消息（person 维度排除我自己）', () => {
  withDb(() => {
...
test('unread：水位推进到最新 → 0；没有 receipts 行 → 对方的全部消息都算未读', () => {
  withDb(() => {
    const db = getDb()
    db.prepare(`UPDATE receipts SET read_upto_seq = 5 WHERE space_id = ? AND person_id = ?`).run(SPACE, ME)
    assert.equal(unreadCount('tok-me').unread, 0, '读到最新 → 没有未读')

    db.prepare(`DELETE FROM receipts WHERE space_id = ? AND person_id = ?`).run(SPACE, ME)
    // 没读过：m2 / m4 / m5 三条（m1 我发的、m3 我另一台设备发的都不算）
    assert.equal(unreadCount('tok-me').unread, 3, '没有 receipts 行 = 全部未读')
  })
})

test('unread：按 person 看（对方的角度与我不同），且不串其他空间', () => {
  withDb(() => {
