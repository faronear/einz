...
/// （[background] 传卡片底色，让门窗"透出底色"而不是糊上一块白），形状也完全可控。
///
/// 尺寸：默认 **48**，等于 _EntryCard 的图标槽位高度。
/// **标签对齐的坑**（2026-09-16）：_EntryCard 里"图标+文字"是**居中**的 Column，两张卡
/// 片只要图标尺寸不同（创建 Icon 44 / 加入房子 46）列高就差 2px → 标签差 1px（老板实测
/// "加入"比"创建"低一点）。现已把图标槽位固定成 48 高并把图标居中放在槽位里，标签位置
/// 与图标尺寸彻底解耦——**两行字严格同高**，且这个 size 只要不超过 48 就不会再影响布局。
/// （另：曾把本体撑到 42px 与锤子等高（需 size 56），图标块变高压低标签，已回退。）
class CuteHouseIcon extends StatelessWidget {
  const CuteHouseIcon({
    super.key,
    required this.color,
    required this.background,
    // 48 = _EntryCard 图标槽位的高度（老板 2026-09-16：小房子再拉高 2px，46→48）。
    // 与槽位同高时墨迹约 44px，不会超出槽位，也不会把标签顶下去。
    this.size = 48,
  });
