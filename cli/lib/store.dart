...
class EntranceStore {
  ...
  List<MessageEnvelope> get pendingEnvelopes {
    ...
    for (final json in pending) {
      try {
        out.add(MessageEnvelope.fromJson(jsonDecode(json) as Map<String, dynamic>));
      } catch (_) {
        // 解析不了的待发信封：跳过（不外发坏内容），**绝不因一条坏数据打崩 TUI**
      }
    }
    return out;
  }
  ...
  ///
  /// 逐条 try/catch：**解析不了的坏信封跳过**，不让 `MessageEnvelope.fromJson`
  /// 的类型转换把整机 TUI 打崩（2026-09-24 老板实测：一条坏历史就让进程直接死，
  /// 连 `/reset` 逃生口都到不了）。**只容灾，不做旧键名归一**——旧键兼容已被明确
  /// 否掉（无外部用户、无老数据留存，见 aimemo/worklog.md 2026-09-25 决策）。
