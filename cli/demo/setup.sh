#!/usr/bin/env bash
# Einz CLI REPL 本机体验环境 —— 一键准备（幂等，可重复运行）。
#
...
cd "$ROOT"
[ -f demo/store-a.json ] || dart run bin/einz.dart init --store demo/store-a.json --device-id dev-a1 >/dev/null
[ -f demo/store-b.json ] || dart run bin/einz.dart init --store demo/store-b.json --device-id dev-b1 >/dev/null
PUB_A="$(dart run bin/einz.dart pubkey --store demo/store-a.json)"
PUB_B="$(dart run bin/einz.dart pubkey --store demo/store-b.json)"
echo "    A=$PUB_A"
echo "    B=$PUB_B"
...
echo "==> 5. 双端认证"
dart run bin/einz.dart auth --store demo/store-a.json --server "$SERVER_URL" >/dev/null
dart run bin/einz.dart auth --store demo/store-b.json --server "$SERVER_URL" >/dev/null
