...
step "2. 停 Server（模拟离线）→ A 离线发送 3 条"
stop_server
for i in 1 2 3; do
...
PENDING="$(py -c "import json;print(len(json.load(open(r'$(wpath "$WORK")/store-a.json'))['pending']))")"
[ "$PENDING" = "3" ] && echo "✅ 离线队列 = 3" || { echo "❌ 期望队列 3 实际 $PENDING"; exit 1; }

step "3. 重启 Server（模拟恢复）→ A sync 自动补发"
start_server
...
step "6. WS 实时：B listen 收到 A 新消息（5 秒窗口）"
