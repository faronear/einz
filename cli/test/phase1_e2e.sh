#!/usr/bin/env bash
# OnlySpace Phase 1 验收：离线发送 → 恢复网络 → 自动补发 → 无重复无乱序
#
# 流程：
...
dart run bin/onlyspace.dart import \
  --store "$WORK/store-b.json" --sealed-file "$WORK/sealed-b.txt" --space-id "space-p1" >/dev/null
...
stop_server
for i in 1 2 3; do
  OUT="$(dart run bin/onlyspace.dart send --store "$WORK/store-a.json" \
...
start_server
SYNC_A="$(dart run bin/onlyspace.dart sync --store "$WORK/store-a.json" --server "http://127.0.0.1:$PORT")"
echo "$SYNC_A"
...
SYNC_B2="$(dart run bin/onlyspace.dart sync --store "$WORK/store-b.json" --server "http://127.0.0.1:$PORT")"
...
( dart run bin/onlyspace.dart listen --store "$WORK/store-b.json" --server "http://127.0.0.1:$PORT" >"$LISTEN_OUT" 2>&1 &
  LISTEN_PID=$!
  sleep 2
  dart run bin/onlyspace.dart send --store "$WORK/store-a.json" \
