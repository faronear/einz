...
#
# 准备（1–2）：pair_up（A 自举登记 + 口令托管 → 邀请码 → B 登记 + 凭口令接入）
...
pair_up "$WORK/a.json" "$WORK/b.json"
echo "    A device_id=$(id_of a.json) person_id=$(field_of a.json person_id)"
echo "    B device_id=$(id_of b.json) person_id=$(field_of b.json person_id)"

step "2. A 发 v1 消息 → B 同步解密（两端同一 Space Key）"
...
assert_contains "$SYNC_B" "v1时代消息" "B 收到并解密 v1 消息"

step "3. 【段 A】B 监听 WS → A 撤销 B → B 收到 device.revoked（App 据此自毁本地数据）"
...
step "5. 【段 B】回归守卫：rotate 命令已撤除（不做 Space Key 轮换，SECURITY.md §3）"
...
step "6. 【段 B】撤销不影响在网设备 A：继续收发 + 本地历史按 key_version 解密"
