#!/usr/bin/env bash
# Einz Phase 4 验收：设备撤销 + Space Key 轮换 + 安全/韧性测试
#
...
PUB_B="$(dart run bin/einz.dart pubkey --store "$WORK/b.json")"
...
dart run bin/einz.dart sync --store "$WORK/b.json" --server "http://127.0.0.1:$PORT" 2>&1 | grep -q "v1时代消息" \
  && echo "✅ B 收到 v1 消息" || { echo "❌ B 未收到 v1"; exit 1; }

step "3. 【段 A】A 监听 WS → A 撤销 B → A 收到 key.rotation"
( dart run bin/einz.dart listen --store "$WORK/a.json" --server "http://127.0.0.1:$PORT" >"$WORK/listen-a.log" 2>&1 &
  LISTEN_PID=$!
  sleep 3
...
dart run bin/einz.dart send --store "$WORK/a.json" --server "http://127.0.0.1:$PORT" --message "v2新消息" >/dev/null
...
stop_server
dart run bin/einz.dart send --store "$WORK/a.json" --server "http://127.0.0.1:$PORT" --message "离线消息" 2>&1 | grep -q "留队" \
  && echo "✅ 离线消息入队" || { echo "❌ 应入队"; exit 1; }
start_server
dart run bin/einz.dart sync --store "$WORK/a.json" --server "http://127.0.0.1:$PORT" 2>&1 | grep -q "补发=1" \
