#!/usr/bin/env python3
# 在线状态回归（老板 2026-09-16 实测）：同一身份的第二台设备 ≠ 对方
#
...
#     ① A/C 两台 TUI：对方始终是 ○（离线），不得出现 "● - #"（绿灯 + 对方名占位）；
#     ② 我的多设备计数生效：两台都显示 "2/2台在线"；
...
def main():
    ...
    try:
        ...
        # 基线：只有 A 一台设备 → 不显示计数；对方未加入 → 左段 "○ - #-"
        # （对方名字：personNames 里没有第二个人时显示中性占位「-」）
        text = wait_screen(m_a, lambda t: '● - #' in t or '台在线' not in t,
                           'A 进入稳定态（对方 ○ 且无设备计数）')
        if '● - #' in text:
            print('FAIL 基线: A 只有一台设备时对方就显示在线:\n', text[-600:])
            sys.exit(1)
        print('A: 基线通过（对方 ○、无设备计数）')

        # ---------- C：A 的第二台设备（选同一身份 Lukas）----------
        ...
        # ① ②：C 上线后两台都该显示 2/2台在线，且对方仍离线
        # （修复后同一人的设备上下线不再互推广播，A 要等下一次 30s 轮询才刷新计数）
        for label, m in (('C', m_c), ('A', m_a)):
            t = wait_screen(m, lambda t: '2/2台在线' in t or '● - #' in t,
                            f'{label} 更新在线状态', timeout=90)
            if '● - #' in t:
                print(f'FAIL {label}: 同身份设备上线后，尚未加入的对方被显示在线:\n', t[-600:])
                sys.exit(1)
            if '2/2台在线' not in t:
                print(f'FAIL {label}: 未显示我的 2/2台在线:\n', t[-600:])
                sys.exit(1)
            print(f'{label}: 2/2台在线 ✓ 且对方仍离线 ✓')
        ...
        for label, m in (('A', m_a), ('C', m_c)):
            wait_screen(m, lambda t: '● Alice' in t, f'{label} 显示 ● Alice', timeout=60)
            print(f'{label}: 对方 Alice 上线 → ● Alice ✓（绿灯未被改坏）')
        print('PASS: 多设备在线状态回归 3 项全过')
    finally:
        for p in spawned:
            try:
                p.kill()
