...
# 输入 steffi → enroll 自举附 partner_name → /health 名称表 personB=steffi。
...
def main():
    ...
    try:
        ...
        # /health 名称表应含 personB=steffi
        names = {}
        for _ in range(20):
            try:
                names = json_load(f'http://127.0.0.1:{port}/health')['person_names']; break
            except Exception: time.sleep(0.3)
        if names.get('personB') != 'steffi':
            print(f'❌ personB 名称未预置: {names}'); return 1
        print(f'✅ /health 名称表 personB={names["personB"]}（预置成功）')
