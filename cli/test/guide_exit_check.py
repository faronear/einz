#!/usr/bin/env python3
# 引导中 /exit 应立即中断回归验证（适配"通道名自动设置"后的新引导流）：
# 新通道 store 为空 → _onboard 自动设默认通道名（不再有"请输入通道名称"问答）
# → 引导首个问答是"请输入您的名字"。在此处输入 /exit：
# 修复前：/exit 只让输入循环退出，引导继续（绑定中…）后才退；
# 修复后：立即结束引导（守卫 return），不再出现"通道与空间绑定中"等后续步骤。
...
def main():
    ...
    out = ''
    try:
        # 1) 冷启动：服务器地址问答（回车沿用）→ _onboard 生成身份+自动默认通道名
        #    → 引导首个问答"请输入您的名字"
        out += drain(master, 8)
        ...
        if '请输入您的名字' not in out:
            ...
            print(out[-800:])
            return 1
        print('✅ 已到达"请输入您的名字"问答（通道名已自动设置，无通道名问答）')
