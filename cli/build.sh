...
#
# 用法:   ./build.sh [输出文件名]      （默认: cli/einz-tui-<平台>）
...
PLATFORM="$(uname -s)"
case "$PLATFORM" in
  Darwin)               OUT="${1:-einz-tui-macos}" ;;
  Linux)                OUT="${1:-einz-tui-linux}" ;;
  MINGW*|MSYS*|CYGWIN*) OUT="${1:-einz-tui.exe}" ;;
  *)                    OUT="${1:-einz-tui}" ;;
esac
...
echo ""
echo "✅ 编译成功: $DIR/${OUT}（$(du -h "$OUT" | cut -f1)）"
echo ""
...
echo ""
echo "运行示例:"
echo "   $DIR/$OUT --store ~/.einz/myeinz.json --server https://einz.tic.cc"
