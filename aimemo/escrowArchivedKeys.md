...
> **状态：`[搁置]`**（2026-09-14 老板决策：**Space Key 轮换不做** → 本方案失去前置，
> 一并搁置；若将来恢复轮换，按 `docs/SECURITY.md` §3.5 的清单再启动，§5 的评审意见仍然有效）
> **补充（2026-09-14 晚）：** 连 `archived_space_keys` 字段本身也已从 store/备份载荷删除
> （无生产者、无存量数据）——恢复轮换时需按本文档重建该字段。
> **补充 2（2026-09-14 稍后）：** 本文档 §3 表格里的接入点 `lock_page._syncEscrow` **已删除**
> （客户端不再本地缓存共享口令）——恢复轮换时该接入点需重新设计，不能照抄表格。
> （原状态：`[待评审]`，老板确认讨论后再转 `[待开发]`）
...
轮换过密钥的空间里，新设备凭共享口令接入后拿不到归档密钥 → 旧 key_version 的
...
归档密钥入箱后，"拿到共享口令"= 能解**该 Space 的全部历史**（不再只是当前密钥）。
