...
## 2026-09-22 多空间 M3：device_uid（服务端设备认知）+ 破坏性入口两档化 + 删掉 clear()

老板两问引出来的三件事：① "服务端能否判断这些虚拟设备属于同一台物理设备" →
答案是能（IP/时间/push token/同名 device_name 都是旁证），但他要**确定性**、且明确
"隐私只需保护到消息层，不必保护到设备层" → 做**显式标识**；② `clear()` 没有调用点
→ 删掉；③ 多空间下"重置设备"藏在单个空间里的语义冲突 → 拆成两档。

### 1. `device_uid`：服务端侧设备认知（§3.7）

- 服务端：`devices` 加可空列 + 索引；create/join 接收（非法形状只留 NULL，不阻断入网）；
  新增 `POST /devices/uid`（会话鉴权、幂等、只写本会话那一行）供**存量设备**补登。
- **绝不出泄**：`/space` 与 `/devices` 都是显式列投影，实测断言两个响应体都不含该字段。
- 客户端：App 在 `app_state` 里惰性生成（`AppLockService.deviceUid()`，安装级；重置/重装
  即轮换）；向导 create/join 带上；ChatPage 进页面时尽力补登一次。TUI 存自己的 store。
- 生产影响：4 台设备各 1 个空间 → 补登后 1:1，行为零变化；等有人同机开第二个空间，
  服务端就能一眼看出那两行属于同一台手机。
...
- `retireDevice(token)` 的既有语义本来就是"退役会话对应的那一行"= 一个空间的虚拟设备，
  正好够用；**"整机退役"不需要 device_uid** —— 客户端 Vault 里有每个空间的 token，
  逐个调即可（这点纠正了我最初把 uid 和退役范围绑在一起的想法）。
- 空间列表长按「移除」也顺手退役该空间那一行（原设计决策④"仅本地移除"会留下 active
  幽灵设备，已修订）。
- 闸门两档共用一个弹窗（本机设备名 + 本机锁屏码）；导航：有列表可退就退回列表，
  列表检测到 Vault 空 → 回向导。

### 3. 删除 `AppLockService.clear()`

M1 之后它没有生产调用点（撤销自毁改走 `removeSpace`），且是"按已知键清单删"的实现
（上一轮修的那个漏删就是这个模式的产物）。清除范围从此只有两家：`removeSpace(spaceId)`
（逐空间）与 `resetLocalData()`（全设备，删 app_state 整表、不会漏键）。3 个单测的
`clear()` 用法分别改到 `clearPackage()` / `clearPlain()`，覆盖的行为不变。

### 验证

- 服务端：`npm run build` + 全套 `npm test` 无回归；新增 `device_uid.test.ts` 3 条。
- App：`flutter analyze` 无 issue；**全量 `flutter test` 167 通过 0 失败**（1 跳过）；
  新增/改写 7 条（空间级退出入口、空间列表退役 + 设备级入口、device_uid 生成与轮换、
  clear 相关改写）。CLI：`dart analyze` 无 issue + store 测试 4 条。
- UI 观感与真机流程（两档破坏性操作、device_uid 回填）仍由老板自测。

## 2026-09-22 术语复核：不引入「入口/关口」，改为文案限定 + 承认设备名 per-space

老板提出重新设计称呼：一个"设备"其实是通往一个空间的**一个入口**，一个空间可有两位伴侣、
多个入口，所以「重置设备」既不重置空间也不重置物理机。问是否把界面里的"设备"改成
"入口/关口（entry/gate）"。

### 结论：不改名词（三条依据）

1. **「入口」在 UI 里已被占用**：`wizardStartTitle`「秘境入口」、`setupEntryTitle`
   「选择秘境入口」= 创建/加入的选择页——占用的是更重要的位置；
2. **「闸门」在代码与文档里已被占用**（破坏性操作的确认闸门，代码注释 5 处）；
   改成 gate 会把既有术语撞烂；
3. 抽象名词不贴合用户心智（用户说"我和她各自的手机和电脑"），且换词后要解释的更多。
   服务端那层抽象（一行 = 一次登记）**在界面上不出现**——设备列表都是 per-space 的。

歧义的真实形态是**动作名**（已在上一条提交里修掉：「重置设备」→ 空间级「退出并清除这个
空间」/ 设备级「清除本设备全部数据」），不是名词。

### 老板的关键澄清：设备名 = 入口名

「我的设备名其实是**我的入口名**，只不过从前一设备一入口一空间，所以用了物理设备名做默认值。」
据此定案：

- 撤回 §2.2 的 ~~Vault 级统一设备名~~（**该决策从未实现**：`VaultPayload.deviceName`
  三个构造点都没传值、恒为空串）→ **删除该字段**，承认设备名归 per-space profile；
- 文案消歧：菜单项「我的设备」→「本机名称」，弹窗标题「我的设备信息」→「本机信息（本秘境）」，
  标签「设备名称/设备公钥」→「本秘境里的名称/公钥」，「已复制设备公钥」→「已复制公钥」，
  并在弹窗顶部加一行：「名称与公钥都属于当前秘境——同一台设备在别的秘境是另一套」。
  （zh + en 均已改，`flutter gen-l10n` 重生成；chat_page_menu_test 4 处断言同步。）

### 遗留（待老板定）

默认名取设备型号（`_autoDeviceName()`）：双方同型号时 TUI `/devices` 里会出现两行同名条目
（App 侧目前没有设备列表界面，影响面只在 TUI）。是否改默认值、或让向导里直接取名？
另：是否要单独落一份**术语表**（物理设备 / 安装 `device_uid` / 登记项 `device_id` / 伴侣 /
秘境 的精确对应），将来别人接手不再绕——本次只在 multiSpaceDesign §2.2 注里记了要点。

### 后续（同一天）：术语表落地 + 向导标题改名 + `device_id` 改名的结论

老板定了三件：

1. **默认名保持现状**（同型号重名只是"用户看起来"的问题，代码里不冲突，用户可自己改名）；
   UI 用词服从用户习惯，**文档与代码里区分清楚层次**。
2. 据此提出：`device_uid` 才是真·设备 id，`device_id` 其实是"空间里的渠道/入口 id"，
   是否把代码里的 `device_id` 改叫 `entry_id`/`channel_id`？有没有 alias 机制可以
   "先并存、等所有客户端换新再删老名字"？
3. 界面文案：`wizardStartTitle`「秘境入口」→「秘境向导」、
   `setupEntryTitle`「选择秘境入口」→「选择秘境」（en 同步：Setup guide / Choose a space）。

**已做**：新建 `docs/GLOSSARY.md`（术语分层的唯一权威：物理设备 / 安装 `device_uid` /
登记项 `devices` 一行 `device_id` / 伴侣 `person_id` / 秘境 `space_id`；含字段对照速查、
命名约定、wire 改名方案与暂不做的理由、物理设备旁证）。`docs/DATABASE.md`、
`docs/PROTOCOL.md` 头部加指针；`server/src/db.ts` 的 devices 建表处加注释
（device_id = 登记项 id，别读成物理设备 id）；设计文档 §3.7 指向术语表。文案两处已改。

**`device_id` → `entry_id` 的结论：暂不做（建议往后放）**。理由与将来的做法写进了
术语表：地基（`X-Protocol-Version` / `?pv=` 硬校验）已有、且 `device_id` 几乎只出现在
响应里（客户端只 revoke 时在路径里发一次），所以双名并存很容易；但缺"判定还有没有老
...
## 2026-09-22 空间入口收敛：菜单合并为一条「切换空间」+ 空间列表去掉破坏性操作

老板三连问把这块理清了：

**①「切换空间」和「空间管理」为什么是两条？进入后界面一模一样。** —— 对，指向同一个页面，
差别只在**导航动作**：从空间列表点进聊天的（栈里有列表）用 `pop` 退回；单空间直达的
（栈里没有列表）用 `push` 新推，而 push 需要 pin，聊天页刻意不持有 pin，所以只能由
"手里有 pin 的入口"（启动门/锁屏页/向导收尾）注入这个回调。实现差异泄漏成了两条菜单
→ **合并成一条「切换空间」**，内部"有列表可退就 pop，否则让注入方 push"；
`spaceListManage` 文案键删除，菜单项/分发/注释同步。

**② 为什么 pin 不能留在聊天页？** —— 聊天页需要的是"已解开的结果"（Space Key 在
`widget.spaceKey` 里），不需要钥匙。PIN 留着的问题在于它是**长期凭证**（重锁后还能再用），
常驻内存＝把"用时输一次"变成"内存里躺着能反复开门的钥匙"（内存转储/崩溃报告/调试器）；
而且现在"PIN 只存在于锁屏页/启动门/向导"是一条能查、能写测试守住的不变式，一旦进聊天页
就会蔓延到弹窗/日志/别的页面；还会让同一个秘密承担多种授权（违反我们"破坏性操作的因子
必须本机独占、不复用其他通道"的规矩）。
顺带给老板提了一个**更干净的中间方案**：把"解锁后的 Vault"放进进程级单例（仅内存），
PIN 依旧只在解锁那一刻存在 → 聊天页可直接一跳切空间，不必"回列表再进"。等他定。
...
- 设备级 `confirmResetDevice` 连同 `resetDeviceTitle/Message/Confirm`、
  ...
  作为"整机清空"原语保留（当前无 UI 调用点）；
- 用户的等价路径 = 逐个空间退出（N 个空间退 N 次，比一键抹掉安全）或卸载重装
  （`ensureFreshInstall` 清残留密钥）；
- 顺带补了一个诚实性修正：空间级退出的服务端退役**失败不再静默**——顶部提示
  「服务端退役未完成，对方设备列表里可能仍留有这台设备」，避免留 active 幽灵
  （`retireSpaceQuietly` 改为返回成败）。

**④ 关于"切换空间直达第一屏"**：老板先提出把小卡片瀑布流并入第一屏，我说明了 pin 约束
（PIN 模式下读/写 Vault 都要 pin，聊天页没有 → 只能由有 pin 的上下文新构造页面），
老板据此拍板**保持独立页面**（主体＝已绑定空间，底部按钮通往第一屏）。

测试：`multi_space_pages_test` 重写（去掉长按/溢出的两条，新增"本页零破坏性操作"与
...
## 2026-09-22 UI 用词定稿：入口 / entrance + 令牌 / token（取代"设备""邀请"）

老板发起第二轮术语重估。结论与执行：

### 1. 设备 → 入口（entrance）

上一轮否决"入口"的理由是它与 UI 里的「秘境入口」（创建/加入选择页）撞车——该处已改名
「秘境向导」，冲突消失。本轮盘点发现真正的问题是**同一个"设备"混着两种含义**：
① 秘境里的那个身份（11 处）② 这台机器（5 处）。于是拆成两套词：

- **入口 / entrance** = 这个秘境里的身份：菜单「入口名称」、弹窗「入口信息（本秘境）」、
  「新入口已开通」、「这个入口未被服务器识别」「对方的入口列表」…
- **本机 / device** = 物理机器：「仅本机生效」「本机锁屏码」「两台设备需要连同一台服务器」。

### 2. 邀请 → 令牌（token）

老板指出关键事实：**令牌不仅能给伴侣，也能给自己另一台设备**（join 流程里那步
「选择我的身份」正是因为这个二义性才存在）——所以「邀请」预设接收方是别人，不准。

- 中文「令牌」、英文 token，与代码 `join_tokens`、协议文档 "join token" 同名（三层对齐）；
- 意外收益：**"填写令牌"不需要再加"码"**（令牌本身就是那段字串），比"邀请码"更干净；
- 菜单项定为「**新入口令牌** / New entrance token」（描述你**拿到的东西**，不承诺
  "已开通新入口"——入口要等对方用了令牌才存在）；「新」用来区别于你已有的入口；
- 配套：弹窗「令牌已生成」+「一次性令牌：凭它可以开通一个新入口（给对方，也可给自己的
  另一台设备）」。整套词自洽：**令牌（手段）→ 入口（结果）**——英文 entrance 原先
  那个别扭点（"invite a new entrance" 不通）也随之消失。

### 各端改动

- **App**（`app_zh.arb` 39 处 + `app_en.arb` 44 处 + gen-l10n）：邀请/设备全套替换；
  「入口名称」「入口信息（本秘境）」「入口公钥」；
- **测试**：8 个测试文件里的断言与注释同步（chat_page_menu / invite_dialog_layout /
  setup_join_passphrase / setup_envelope_verify / widget_test / golden_render，
  含 `find.text('邀请')`→`'新入口令牌'` 这类）；
- **TUI**（`cli/bin/einz_tui.dart`）：15 处用户可见文案（`/invite` 帮助行、输入提示、
  校验中/失败/限流提示、"等待令牌输入…"）+ 注释里的复合词；
- **文档**：`docs/GLOSSARY.md` 分层定义表加「令牌」一行、"界面用词"条整条重写
  （含为什么改、修订理由）。

验证：`flutter analyze` 无 issue、全量 `flutter test` **175 通过 0 失败**、
`dart analyze`（cli）无 issue。
...
## 2026-09-23 · 术语：界面用词「入口」→「通道」（英文仍 entrance）

- 老板过夜后提的新词：通道比入口更朗朗上口、更贴"本机 ⇄ 秘境"的链路想象。评估后**采纳**：
  ①「入口」在中文代码里早已是 entry point 通用词（语音入口 / 服务器入口 / 接入口令），一词两义；
  ②登记项本质是"安装 × 秘境"二元组，「一条通道」自带两端（本机端的密钥与会话 + 秘境端的身份），
  而「入口」只锚住秘境那一端；③现有文案的动词（开通 / 销毁 / 撤销）本来就是给「通道」配的。
- **代价（已写进 GLOSSARY）**：「通道」偏"连接"的画面，弱化了"身处秘境之中"的体验隐喻
  （「入口」更贴"走进去"）。取舍上取"更贴当前阶段要管理的对象"——多空间下用户管的就是
  "哪条线连到哪个秘境"，销毁/撤销本质是剪线。
- **英文两个候选都否掉**：`passage` 与同屏出现的 `passphrase`（共享口令）同根相近；`channel`
  会被读成 Slack / WebSocket / Go channel → 英文保留 `entrance`，**中英故意不同名**。
- **技术机制的「通道」不改名**：老板提过改叫"信道"，我反对——「平台通道」是 Flutter 官方译名、
  worklog 历史不可改、"厂商信道"读着怪；真正会混的只有密钥 / 撤销语境那几处，已改称
  **「链路」**（SECURITY 5 处 / E2EE 1 / KEY_ESCROW 1），GLOSSARY 加消歧一行。
- **实操陷阱**：不能全局替换——「接入口令」是"接入 + 口令"（会变成"接通通道令"），
  「语音入口 / 拍照入口 / 服务器入口 / 入口页」是 entry point，全部保留。
- 落地 13 文件（arb + 生成共 18 条 UI 串、测试断言、cli TUI 4 处、chat_page / reset_device 注释、
  GLOSSARY / SECURITY / E2EE / KEY_ESCROW / DEPLOYMENT）：commit `e9bd61b`。
  验证：flutter analyze 无 issue、flutter test 175 通过、cli dart analyze 无 issue。**未 push**。
- **补（同日稍后）**：老板拍板同步活的设计文档 → `multiSpaceDesign` 8 处 + `projectPlan` 2 处
  （「入口名」→「通道名」、「销毁本秘境入口」→「销毁本秘境通道」、「入口管理」→「通道管理」、
...
## 2026-09-23 · TUI 动作类交互「留空回车 = 取消」（`_promptAction`）+ 修探针腐烂

老板反馈：`/revoke` 进去后只能输通道序号、输乱码（还报"无效"）或 `/exit`（杀掉整个 App），
**无法简单地退出这个动作**。

**根因**：`required: true` 是给**向导必填**做的（留空回车拦住，防误提交空名字/空口令），
后来被**动作类** prompt 复用了 → 用户被困在动作里。`/passphrase` 更狠：老/新口令都是
...
**改动 8 处**：`/revoke` 3 步（序号 / yes / 口令）、`/reset` 2 步（通道名 / 锁屏码）、
`/passphrase` 3 步（老口令 / 新口令 / 引导里的托管口令设置——留空=跳过，之后可 `/passphrase`）。
提示语统一加"（留空取消）"；顺手删掉两处依赖"required 拦空回车"的不可达防御分支。
**首启向导的 `c/j` 选择保留 required**（老板定：那一步没有"回到聊天"可言）。

**拒绝的方案**：显式取消词（通道名允许字母/数字/`_`/`-`，`q` 可能真是通道名，得处理优先级）；
Esc（raw 循环里 ↑↓ 输入历史就是 `\x1b[A`/`\x1b[B`，都以 ESC 开头，要区分单 Esc 与转义序列
...
- 顺手修了 5 个探针里 2026-09-22 改名遗留的旧串（`密保口令`→`共享口令`、`邀请码`→`开通码`、
  `我是谁`→`完整输入我的名字`、`此项不能为空`→`请输入内容`、`同空间 N 台`→`条`）——
...
1. **向导必填的空回车必须"静默拒绝"，不是"打一条提示"**。老板原话："如果不输入而直接回车，就拒绝接受，
   ...
   追加一条 `⚠️ 请输入内容`，把提问往上顶一行、屏幕跟着跳。现在改为：清空输入行、**什么都不打印**、
   原地继续等（提示还在上面、光标还在输入行，用户自然知道要接着输）。
2. **向导里唯一允许空回车跳过的是锁屏码**（老板）：所以我把上一条里"引导中的托管口令设置"从
   `_promptAction`（留空=跳过）**改回 `_prompt` + `required: true`** —— 那是向导的一步，不是"动作"。
   最终边界：向导必填（秘境入口 c/j、名字、性别、开通码、身份选择、共享口令、口令接入）→ 静默拒绝；
...
## 2026-09-23 · App l10n 死键清理（21 个）+ 开通码弹窗去冗余按钮 + 身份弹窗字段去人称

老板三件（我上一轮只报了 2 个死键，他让"删掉死键"，于是做了全量扫描）：

**① 死键不是 2 个，是 21 个**。扫法（下次可复用）：
按 `\b整词\b` 在 `app/lib` + `app/test` 的 .dart 里找每个 arb key（**排除 l10n/ 目录**，那里是生成物），
再用**子串 grep 复核**——只有 `chatPageMsgSending` 有子串命中，查明是另一个键 `chatPageMsgSendingTap`
（我第一版抽查用子串 grep，反被这个假阳性骗了一次，记下来）。
删完 `flutter gen-l10n` + `flutter analyze` **通过**即是"无人引用"的证明（有引用会直接报找不到 getter）。
删后复查剩余死键 = **0**。清单见 commit 5663278（多数是 v1 收敛后遗留的 wizard/身份页/媒体失败文案）。

**② 生成开通码弹窗**：原来底部两个纯文字按钮 Copy / Close，没主次。老板建议"两个都不要（点弹窗外自动关），
或者只留一个关闭"。我选了**只留 Close**，理由：点弹窗外/返回键**本来就能关**（`showDialog` 默认
`barrierDismissible: true`），所以"两个都不要"并不新增能力，反而少了给"不知道能点外面"的人的明路；
而底部那个 Copy 复制的是**链接**（token/链接两行各自已有带 tooltip 的复制图标 + 顶部提示），删掉零损失。
（要真改成"两个都不要"是两行的事。）
...
**老板的原始动机（动作路径，不是落点）**：从对话页切空间要三步——
① 点汉堡菜单 ② 眼睛扫描菜单项 ③ 点「切换我的秘境」 ④ 再挑目标。微信里是
冷启动进列表 → 选人 → 之后「返回箭头 → 选人」两步。他要的是这个"少一次跳转 +
少一次扫菜单"的体感。

**先否掉的方案：把弹层做成独立页面 + 冷启动先落列表（原提案 A+B）**。
拆成两件事看才清楚，成本差很大：

- **(A) 弹层 → 独立页**：便宜、中性偏好；
- **(B) 冷启动多空间先落列表**：**是 2026-09-22「冷启动直接进上次的空间」决策的翻转**，
  且「对话页左上角返回箭头回列表」**只有 B 才成立**——返回箭头 = `pop`，列表必须在栈里、
  且在对话页**下面**；而当前对话页一直是 `pushReplacement` 出来的栈底（`chat_entry` /
  `lock_page._enterChat` / `space_switcher.switchToSpace`），压根没有返回箭头。
  这正是 2026-09-22 合并两条菜单时遇到的那个岔口（"有列表可退就 pop，否则得 push，
  而 push 要 pin"）。
- B 的**实打实风险**：**后台重锁空窗**——重锁逻辑挂在 `ChatPage.didChangeAppLifecycleState`，
  多空间先落列表时列表是栈底、没有 ChatPage 活着 → 切后台回来不触发覆盖锁，会停在
  "解锁后的列表"上（露出空间名/头像/未读）。要做得把重锁上移到 app 级。
- 结论：对 1:1 私密通讯，「打开即进上次那个人」比「先选空间」值钱；B 是 Slack 式"工作区"
  产品的手感，与「秘境」隐喻未必更贴。老板同意 B 不值得。

**落地（老板提的，最省的一档）**：状态条（`chatPageStatusBar`）里**对方在线圆点/名字旁**
加一个下拉箭头，整簇可点 → 一步打开现有的「选择秘境」弹层。复用
`chat_page._openSpacePicker`（弹层 + `switchToSpace` 全都现成），**零路由 / 零冷启动 /
...
- (A)+(B) 的结构**方向上略微利好**"PIN 不入对话页"：切换/管理回到"门侧上下文"，
  对话页退化成纯叶子。**但代价**是列表页会长期活在栈里、天然要读整个 Vault，
  若为了"新建/退出别反复输 PIN"顺手指给它 PIN，就重演 09-22 想避免的事。
...
1. **汉堡菜单**黄条 + `right overflowed by 69 pixels`：菜单带值行原先是
   ...
   右对齐），7 处带值行统一换。短值外观不变，菜单仍按内容自适应宽度，180 只是上限。
2. **状态条**名字越过中线顶到对面名字旁、并冲出胶囊条到屏幕边缘：改为**左右各一个
   `Expanded`（各占一半）** + 两侧名字 `Flexible` + 省略号；圆点与箭头是固定项永不被压掉。

**回归测试**：`chat_page_menu_test` 新增两条（箭头一步开弹层、长名字不溢出）；
后者**已反向验证**——把 `kMenuValueMaxWidth` 临时放到 100000，测试立刻报
`RenderFlex overflowed by 417 pixels`，还原即绿（证明这条测试真能抓到老 bug）。
新用例刻意**不 assert 按钮文案**（只认弹层标题），免得跟老板的文案润色互相打架。
全量 `flutter test` **188 过 1 跳过**；`multi_space_pages_test` 那条
「新建/加入秘境」旧文案断言是**既有失败**（arb 已是「添加秘境」），本次未处理。

## 2026-09-24 · 顶栏与空间卡片的连续打磨（commit `63a190f` `571120c` `e361383` `4942315` `0018036`）

老板一口气提了一批 UI 细节，都是"看着不对就改"式的迭代。按位置归类：

### 顶栏（`chat_page.dart`）

- **阅后即焚标记**（`63a190f`）：当前空间开了焚毁（非 Off）时，在 actions **最左**
  （锁屏按钮/汉堡菜单的左侧）显示 `沙漏 + 档位`，**点击直接进档位弹层**。
  - 档位文字只用量纲单位 **d/h/m/s**（取能整除的最大单位），**不用中文单位**——横向省、
    且不随界面语言变化。
  - 图标经历了三版：`local_fire_department`（火苗）→ 老板否掉（不好看）→
    **沙漏**，并要求"和消息气泡里的一样、和锁屏图标一样大"。气泡用的是
    `_BurnHourglass`（在 `hourglass_top`/`hourglass_bottom` 间**翻转**），
    顶栏要**静态**，故取 `Icons.hourglass_top`、size 24（= IconButton 默认）。
  - 颜色也改过一版：我原先给的是暖橙/红字，老板要求"就用和锁屏图标一样的字体"→
    **不指定颜色**（继承顶栏前景色）。
- **状态条左侧做成"可按芯片"**（`63a190f`）：底色比胶囊深一档 + 右圆角 + 实心下拉三角
  （线箭头与文字笔画相近、不好区分，故用实心），整块可点开弹层。实现要点：
  - 内边距从胶囊移到左右两块各自持有，外层 `clipBehavior: Clip.antiAlias` →
    芯片的**上/下/左三边与胶囊边框完全贴合**，方角被胶囊圆角裁掉。
  - 父级 Column 不限高，`CrossAxisAlignment.stretch` 会得到 `Infinity` 高度 →
    必须套 `IntrinsicHeight` 给这行确定高度。
- **单空间不给切换暗示**（`4942315`）：只有 1 个空间时，状态条左侧退化成纯
  「圆点 + 名字」，无箭头、无底色、不可点。老板的理由："单空间常常就是只想和某一个人用，
  不该暗示这里能切换；真要加空间去汉堡菜单里找"。判断依据 = 已加入的空间数
  （取内存里已解锁的 `VaultSession`，不触安全存储、不需要 PIN）——老板认可"加第二个空间后
  箭头立刻出现"这个行为，不需要额外的"用户是否用过切换"偏好标记。
- **logo + 品牌名可点**（`0018036`）：等同菜单里的「关于秘境」（老板："别闲着"）。
  按住高亮原先紧贴标题文字右缘 → 加 `Padding(horizontal: 8)`，并把 AppBar 的
  `titleSpacing` 从 16 收到 **8** 相抵，**logo 位置一点没变**但高亮四周有了边距。

### 空间卡片（`widgets/space_switcher.dart`，`571120c`）

- 头像半径 18 → **24**，名字字号 13 → **15**（老板："空间的核心就是对方是谁，可以强调"）。
- **正方形 + 按屏宽反算边长**：原先固定 104→120 都试过，"2 张空太多、3 张放不下"。
  最终用 `LayoutBuilder` 取弹层可用宽度，`(宽度 − 2×间距) / 3` **向下取整** →
  iPhone 16（393）得 112，`3×112+2×12=360 ≤ 361`，一行正好 3 张；不满 3 张的行靠左
  （`Wrap` 默认 `WrapAlignment.start`）。桌面兜底：卡宽上限 160，否则宽窗口里会算成几百像素。
- **内容居中**：`Stack` 默认 `StackFit.loose` 会让 `Column` 缩到内容宽度并被摆到左上角
  （看着像"靠左"）→ 加 `fit: StackFit.expand` 才真正居中。
- **勾的位置反复过一轮**：右下角（会盖住名字）→ 右上角（**与未读角标撞位**）→
  最终 **左上角**，未读角标回**右上角**，两角各占一个。
- 防撑破：名字 `maxLines: 2` + `ellipsis` + `Flexible`（大字号下也只会被压缩，不顶破卡片）。

### 「添加秘境」按钮（`widgets/space_switcher.dart`，`e361383`）

- 常态就给**淡灰底**（黑 5% + 圆角 12），提示"这里可以点"；鼠标悬浮/按住再到 10%/14%
  渐变深色。原先只有按住才有底色，看不出可点。
- 去掉与空间卡片之间的 `Divider`。

### 档位弹层可滚动（`widgets/option_picker_sheet.dart`，`0018036`）

- 我在做焚毁标记测试时发现的**既有隐患**：该弹层的 Column 不可滚动，6 档 ≈ 354px，
  而底部弹层最高只有屏高的 9/16 → **窗口高 < ~628px 必溢**（桌面小窗/横屏手机）。
- 改为：标题与「提交」按钮固定，选项区包 `Flexible` + `SingleChildScrollView`。
  内容矮时仍**贴合内容高度**（不强行撑满），内容高时才滚。

### 测试与验证

- 新增 `test/option_picker_sheet_test.dart`：8 档在默认 800×600 下不溢出 + 拖动能滚；
  矮内容贴合高度。
- `chat_page_menu_test` 新增：状态条箭头（多空间）、单空间无箭头/无芯片、焚毁标记、
  点标题进 `AboutPage`、高亮宽 = 内容宽 + 16px（钉住这次的边距）。
- `multi_space_pages_test` 新增：长名字不撑破卡片、按屏宽反算一行正好 3 张。
- 长名字溢出那条**做了反向验证**：把上限临时放大 → 立刻报 `overflowed by 417 pixels`。
- 全量 `flutter test`：**198 过 / 1 跳过 / 0 失败**。

### 流程约定（老板 2026-09-24 定）

**今后 worklog 必须和代码同一次提交一起更新**，不事后补。此前我习惯先提交代码、
...
## 2026-09-25 决策：撤回"旧键名归一"，只留容灾（两次崩溃的性质认定）

**背景**：昨晚 CLI、今天 App 这两次崩（`sender_device_id` / `device_id` → null cast）
都不是真实故障——**是我拿老数据跑新代码刻意试出来的**。外部无用户、无真机，代码只在
老板自己几台测试机上，而老板的实际计划是把所有老数据**手工删掉**。
...
- 第三处（`message_repository` 读历史信封走 `MessageEnvelope.fromJson`）→ **不修**，同一前提。

**代价认识**：这样代码库里就没有任何"读旧数据"的兜底了。将来若再有人把老数据摆上来
（老安装包、备份还原、换机），症状会是 App 解锁不了 / TUI 启动即崩；那条路径上 App 连
"清除本机数据"逃生口都点不到。**这是已知且被接受的取舍**，不是漏洞。

**验证**：app `flutter test` 200 全过；cli `dart analyze` 无 issue + `dart test` 22 例全过。
...
## 2026-09-25 App 界面一批调整（通道列表卡片化 / 关于秘境改弹层 / 状态条「邀请加入」）

本批是老板连着一轮轮口述的界面微调，跨度多个提交（`3d8df1b` → `d312944`）。**注**：前几个
提交只改了代码、worklog 没跟上（老板 2026-09-24 定的"worklog 与代码同一次提交"没做到），
这里回填。

### 通道列表（`chat_page.dart` `_showEntranceListSheet`）

- 菜单项「通道列表」→ **「更多通道」**（`095c018`）；同时**删掉菜单里的「生成开通码」**，
  入口收进弹层的「新建通道」，避免两个入口语义重叠。
- 列表**必含当前通道**（`3d8df1b`）——即使只有自己一条线也要看得见"我是谁"。
- 卡片化（`b617ed1` / `a83d64b`）：**一行 3 张**、边长按弹层可用宽度反算（同空间卡口径）；
  两行布局 = 名称 + 本机标记 / 状态红绿灯 + since；卡片与「新建通道」之间补 12 留白。
- **本机标记：文字标签 → 绿勾**（`Icons.check_circle` 14px，老板 2026-09-25 实测后改）：
  原先名称旁挂「本机 / This device」小灰标签，**英文那个在一行三张的卡里要吃掉近半张宽度**，
  名字被挤到只剩两三个字符。绿勾占 14px，且与空间卡片的"当前这个"用同一个符号。
  **本机卡恒列第一位**，是区分依据之一（测试里钉住顺序）。顺带删掉四个从没被引用的
  l10n 死键（`chatPageEntranceTagLocal/Online/Offline/Revoked`，文字标签时代的残留）。
- 「新建通道」按钮与「添加秘境」**同款外观**（常态淡灰底 5% + 圆角 12 + 图标文字居中）。
- 红绿灯三态：绿=在线（`connected_at` 非 null）、红=离线、**灰=已撤销**；已撤销照列
  （与 CLI `/entrances` 同口径，藏着不显示反而像凭空消失）。

### 「关于秘境」独立页 → 底部弹层（`b9d754d`）

- 新增 `widgets/about_sheet.dart`（`AboutSheet.show`），删除 `about_page.dart`。
- 理由：内容只有版本号 + 服务器地址 + 一句话说明，不值得占一整个 route。
- 三处菜单（对话页 / 向导页 / 锁屏页）都改指弹层，入口行为不变。

### 向导 PIN 步骤按钮（`d312944`）

- 本机**已有**锁屏码时（多空间再加一个空间）`_pinStepSkippable` 判为 false —— 底部按钮恒为
  「下一步」，不再显示「跳过」（锁已经在，没有"跳过"可言之）。

### 状态条「邀请加入」链接（本提交）

- 对方尚未加入时，在**对方芯片右侧、芯片之外**并排一个淡色小字链接（间距 15）。放芯片外是
  老板明确要求：点它必须是"邀请"，不能和"切换空间"混在一个命中区里。
- 点击 → 复用 `_showInviteDialog`（生成开通码弹窗，同「新建通道」）。
- **圆角 12**，与「添加秘境」/「新建通道」那类可点击文案同口径（初版 8 偏方，老板指出）。
- **判定口径 = 有"在用"通道**（`status == 'active'`），不是"通道行存在"：撤销不删行、只改
  status（`server/src/entrances.ts`），拿"行存在"当已加入会让密友重置设备后**不给邀请入口**
  ——恰恰丢了最该邀请的那一刻。status 缺省（老服务端）按在用算，与通道列表弹层同口径。
- **`_peerJoined` 用 `bool?`**：`null`（还没问到）**不显示**链接。否则每次进聊天页都要先闪
  一下；请求失败走 catch 保持原状态 → 离线时更会常驻一个假邀请。
- 新增 `test/peer_invite_link_test.dart`：已加入（离线）不显示 / 已撤销要显示 / 只有自己不 /
  请求在途不显示 / 拉不到不显示。
- 文案：`chatPageInviteJoinLink`（zh「邀请加入」/ en「Invite」）。

### 同批文案（另一次提交）

- `welcomeDialogMessage`：中文改为"…确保绝对隐私。现在就进入秘境，开始聊天吧！"，
  英文 `Enter now and start chatting!`。
- `setupCreateShareTitle`：收敛为"快把邀请链接发给伴侣，一起体验秘境："（英文同上），
  句尾冒号引出下方的链接/二维码。
- golden 测试同步新文案。
...
- **「我的」那块也做成可点**（老板 2026-09-25："我的状态信息也别闲着"）：点开同一个
  「更多通道」弹层。与 invite 不同，**常态不留底色**，只有悬浮（黑 5%）/按住（黑 8%）才
  变色——因为底是 0，这两档比 invite 那套（5% 底上叠 10%/14%）淡一档。
  实现上套了 `Align(centerRight)` 把可点区收到内容大小：不套的话外壳会撑满右半边，
  悬浮高亮会变成横贯半个胶囊的长条。外观与之前一致（原来也是既右对齐又占满半边）。
  内边距 10/6/16/6 → **16/6/16/6**（老板 2026-09-25：左边留白比右边那颗状态灯的留白小，
  看着偏）。
...
## 2026-09-25 决策：英文文案**一律句首大写**，不做 Title Case（已 reset，代码回到原样）

老板先要求"页面/弹层/弹窗/菜单/按钮的英文标题改 Title Case（每词首字母大写）"，我照做了
（56 条）；随后在边界上连着出问题（tooltip 算不算、"Scan token QR code" 一身两用），
老板问："干脆全都句首字母大写，是否全局更简单？" **我的判断：是，更简单，界面上也更好**，
老板采纳，并要求把这两次提交 **reset 掉、当没发生过**，只留下这条决策。
...
- **一条规则**：首字母大写，其余按正常英文写。不用再判断角色。
- **层级已经靠字号字重撑着**：弹层标题 16/w600、菜单行 13、说明 12 灰 —— 大写不是用来
  区分层级的，去掉不掉东西。
- **现状是混着的**：`My Identity`（标题）紧挨 `Partner's name`（字段标签），反而乱。
- **更好扫读**：句首大写的单词形状差异更大；用户英文非母语时正常句子更易读。
- **是主流不是将就**：Material 3 规范就是"所有 UI 文字用句首大写"，Apple HIG 也偏这个。

### 例外（两案都一样，照旧）

品牌与缩写保持自身大小写：`Einz` / `My Einz` / `About Einz` / `PIN` / `QR` / `OK` /
`RESET` / `English`。

### 执行痕迹

- 已 `git reset --hard ad7ad25`，两次提交（`i18n(en): 标题类英文文案改 Title Case`、
  `i18n(en): tooltip 不做 Title Case…`）连同它们带的 worklog 一并撤销；
  **app 代码与英文文案回到原样**（"Create a space" / "About me" / "Token created" …）。
- 顺带说明：reset 前工作区里老板自己改的一条
  `wizardJoinPassphraseRequired`（"Please enter…" → "Enter…"）也被一并清掉，
  老板说"过后再加"，**未恢复**。
- 这条规则已写进长期记忆（`memory/feedback_english_sentence_case.md`）。

## 2026-09-25 状态条「邀请加入」：去掉常驻灰底，改成蓝色链接 + 悬浮/按住才出底色

老板："状态条里太多可点击按钮了不好看，平时就设一个蓝色的链接颜色来暗示可点击，
鼠标悬浮或点击再出现背景色效果，就像我的状态芯片一样。"

- 常态：`Material(color: transparent)` + 文字 **品牌深蓝 `#2271F7`**（与开通码弹窗里
  那条邀请链接同一个蓝，语义同源）。
- 悬浮/按住：黑 5% / 8%，**与「我的」状态芯片同一档**（那块也没有常驻底色）。
- 弧角 24、内边距用芯片那个 `pad`、与芯片间距 8 都不变。

**为什么不用「添加秘境」那套常驻淡灰底**：那条口径是给"独立成行的大按钮"用的
（常态就要看得出能点）；状态条这行已经有对方芯片（可切换）、我的状态（可点开更多通道）
两处命中区，再挂一块灰底就成了第三个按钮 —— 老板的判断是对的，密度太高。

## 2026-09-25 阅后即焚弹层：大标题下加一行居中备注（待老板体验后提交）

老板："在大标题下放一句也居中的解释性备注：『倒计时删除已阅读的新消息』，注意页面美观，
大标题、小备注、内容主体之间简洁自然有区分度。"

- `OptionPickerSheet` 加可选 `note`（null = 不显示），只有阅后即焚传
  （`chatPageBurnNote`；界面语言/附件存储不变）。
- 三级区分：
  | 层 | 字号/字重 | 颜色 | 位置 |
  |---|---|---|---|
  | 标题 | 16 / w600 | onSurface | 居中，padding 16/14/16/10 |
  | 备注 | 12 / 常规 | **outline**（比 onSurfaceVariant 更淡） | 居中，padding 24/0/24/14，行高 1.4 |
  | 选项 | 15 / w600 + 12 说明 | 说明用 onSurfaceVariant | 靠左、可滚动区 |
- 间距表意：备注**贴着标题**（上 0，只隔标题自身的下 10）、**离选项远**（下 14）→
  一眼看出它归标题那一组，不是内容主体。
- 备注在 `Flexible(SingleChildScrollView)` **之外**，滚动时标题+备注固定不动。
- `textAlign: TextAlign.center` 必须写：`Center` 只管整块居中，不管换行后行内对齐
  （中英文都可能折成两行）。
- 英文按 2026-09-25 定的句首大写规则：
  "New messages are deleted on a countdown once read"。
- 备注颜色先用了 `onSurfaceVariant`，老板看后说"换更淡" → 改 `scheme.outline`
  （弹窗内"大标题下的小字说明"本就用这一档，如 `_SetLockDialog` 的提示）。

## 2026-09-25 状态条：去掉 invite 与芯片之间的间隙（底色边缘相碰）

去掉常驻灰底之后，8px 的间隙看着像断开的 → 老板要求"让它们的背景色（当点击时）的
边缘碰到一起"。做法：删掉 invite 外面那层 `Padding(left: 8)`，两块直接相邻。

- 文字之间的距离**不靠这个间隙**，仍由两侧各自的 `pad` 撑着（芯片右 10 + 链接左 16）。
- 两块都是圆角（芯片右 24 / 链接 24），所以严格说是**在竖直中点相切**，上下角各留
...
## 2026-09-25 气泡字号：正文 15→16、时间戳/焚毁标签 10→11、发送状态标签 10→11

老板："15 还是比微信小一圈。正文字体改 16，时间戳、倒计时标签改 11，
发送状态点击重发/重发中 也改 11。"

- `kMessageFontSize` 15 → **16**（`chat_page.dart:152`）：只管气泡内**跟随默认样式**的文字
  （正文、文件名）——引用块 13 / 时间戳 / 状态标签各自显式设字号，不受它影响。
- 时间戳、焚毁倒计时标签：10 → **11**（同一行里的两个 Text）。
- 发送状态「点击重发 / 重发中…」：10 → **11**（失败态与发送中态各一处）。
- 全文件 `fontSize: 10` 只有这四处，改完清零；`.dart` 里别处没有 10px，测试也没依赖字号。
- `flutter analyze` 无告警，全量 217 项通过。
...
4. 补一句：「不要 since 这个词，太占地方，直接写时间即可」（App 卡去掉前缀；TUI 保留
   —— 那是老板 2026-09-17 点名要的，一行也放得下）。

### 先说缺口：离线卡片**根本没有数据可显示**

`server/src/ws.ts` 断开时 `UPDATE entrances SET last_seen = 0`，**把时刻丢掉了**（心跳超时走的
也是同一个 `close` 事件，同样归零）。于是干净下线的通道 `stamp=0` → App/CLI 都不渲染文字 →
卡片矮一截。精确断线时刻只存在于审计表 `connection_events.at_ms`，而 `GET /entrances` 没带出来
——正是 worklog 2026-09-17 记的「要的话是服务端加字段，另议」，这次就是"另议"。

**方案（选 A）**：`entrances` 加 `offline_since INTEGER`（建连清空、断开写时刻、撤销/退役写撤销
时刻），`/entrances` 返回它；**显示层取 `max(last_seen, offline_since)`** —— 干净断开时
`last_seen` 归零、只剩 `offline_since`；服务端重启这类"close 没跑到"的情况反而是 `last_seen`
更新，取较晚者天然两者都对。
另做**一次性回填**（`db.ts` 迁移）：从 `connection_events` 给"现在已离线、`last_seen` 已归零、
`offline_since` 还空"的行补上——不回填的话老板此刻盯着的几张离线卡要等各自重连一次才见文字。
这是唯一一次从审计表取业务值（迁移期一次性历史回填，不是长期读路径），幂等。

**被否的两条**：① 运行时用子查询从 `connection_events` 取值——把审计表拉进业务读路径，
`db.ts` 明写审计"不参与业务语义"，不划算；② 断开时不归零 `last_seen` 而是留断线时刻
——那会推翻「`last_seen = 0` ⟺ 离线」这个被 App/CLI/测试/注释共同依赖的不变式（连审计注释、
`entrance_retire.test.ts` 都在断言它），风险大于收益。
...
先写了 `ConstrainedBox(minHeight: 15)` 兜"没有时间"的第二行，测试立刻红：同样两张卡
**56 vs 57**。原因——M3 `bodyMedium` 的 `height: 1.43` 会跟着 `TextStyle.merge` 一起继承，
11px 时间文字实际量出 **≈15.7px**（16），而空行的下限是 15 → 差 1px；而 15 这个数在
真机字体（行高≈1.17）下又是另一个值，属于**跟着字体漂的幻数**。
改法：第二行**恒定渲染文字**，没有时间时给一个**空格**（不是空串——空串在部分平台量出 0 高）。
同行同款 → 天然等高，不需要幻数。测试里补了一例「两个时间字段都为空的存量行也要与别的等高」
（撤掉这个改法即红）。

### 测试

- 服务端新增 `peer_status.test.ts` 一例：断开后 `offline_since` 落在断开前后、`last_seen` 仍归零、
  重连清空；`entrance_retire.test.ts` 补断言退役也写时刻。
- App `entrance_list_sheet_test.dart`：fixture 增加「离线（`last_seen=0` + `offline_since`）」一张卡
  （红点 + 有时间），断言四张卡**等高**、时间文本**不带 since**、两种角标到各自卡片右上角的内缩量
  一致（= 固定位，不随名称长短跑）、已撤销卡有蒙版、正常卡没有。
- 文档同步：`DATABASE.md`（新列）、`PROTOCOL.md` §7.1（字段与取值口径）、§7.2.1（退役动作）。
...
## 2026-09-26 TUI /revoke 二次确认：输入 `yes` → 抄一遍目标通道名

老板："TUI /revoke 时「确认请输入 yes（留空或其它任意输入取消）」改成要求输入待撤销的
通道的名称"。

**为什么这条值得改**（我补的理由）：`yes` 是固定词，闭着眼也能敲过去，起不到"看清撤的是
哪一台"的作用；抄一遍名字逼人把屏幕上那一行读进来。选错序号就是**不可逆的数据销毁**
（对方客户端会清空本地数据，含历史消息与附件）。

**改法照搬 /reset 的既有口径**（那条闸门早就这么做了，连取舍都写在注释里）：

- 有名字 → `❓ 请输入通道名「X」以确认撤销，或直接回车取消:`
- 没名字 → 退化为固定确认词 `REVOKE`。**不**降级成 entranceId——抄一串 UUID 只会制造
  新的抄错机会（`/reset` 的老注释已立过这条规矩）。
- 留空 → 「✅ 已取消（未做任何改动）」；输入不符 → 「✅ 已取消（输入不符，未做任何改动）」
  （原来两种共用一个分支，现在按 /reset 分成两句，用户能分清"我放弃了"与"我抄错了"）。
- 精确比对（大小写敏感）：破坏性操作的闸门不放松。
- `_EntranceRow` 加 `hasName`：要知道 label 到底是真名字还是回退出来的 id，才能决定
  抄名字还是抄固定词。
- 顺手修一处隐患：`_fetchEntranceRows` 的 `entrance_name` 没 trim，而确认步要**逐字**比
  对 → 名字里带空白字符就成了"怎么抄都不对"的坑（App 侧本来就是 trim 后再显示/比对，
  CLI 这次对齐）。

**探针同步** `cli/test/revoke_command_check.py`：对方通道名是**宿主名派生**的，探针预知不了，
只能从列表帧里读回来 → 新增 `parse_peer_name()`（正则 `\d+\)\s*[🟢⚪]\s*(.+?)\s*\[member\]`）；
三处 `确认请输入 yes` + `send('yes')` 换成新提示 + 抄名字；**新增 ③c 一例专门证明旧的固定词
`yes` 已被拒**（→「输入不符」且未做任何改动）。头注释与 `aimemo/projectPlan.md` 同步。

**验证（真跑）**：`flutter analyze`(cli) 干净；`revoke_command_check.py` 全程通过——它自己拉
临时 server + 两个真 TUI（pty），实际打印「对方通道名 =「DoomBase」」，
①/②/③/③b/③c/④/⑤ 七段断言全绿。

## 2026-09-26 一行三张卡片的边长上限 = 弹层最大宽度

老板："桌面版弹层（More entrances / Switch my space）有个最大宽度，窗口拉大到一定地步
弹层就不再长；那么把一行三张卡片的最大宽度设为和弹层最大宽度一致"。

### 先量清楚"弹层的最大宽度"是多少

写了临时用例在 800 / 1600 / 480 宽的视口里量：`BottomSheet` 组件本身铺满整屏，但**内容**
（Material）被限在 **640 且居中**——800 宽时内容区 80→720，1600 宽时 480→1120。
即 Flutter M3 给 modal bottom sheet 的默认上限（我们没在 theme 里另设 constraint）。
减去弹层内容左右各 16 的内边距 = **608 可用宽度**。

### 原来错在哪

两处 `_entranceCardMaxSize` / `_kCardMaxSize` 都硬写 **160**，注释的理由是"不设的话窗口一宽
卡片会离谱地大"——但那条担心建立在"弹层会随窗口无限变宽"上，而弹层其实卡在 640。
于是宽窗口下三张卡只铺到 3×160+2×12 = **504**，弹层 608 里右边空了一百多像素。

### 改法

把上限从硬编码改成**由弹层最大宽度反算**（两边各一份，与既有的 `12`/`3` 一样是双份口径，
注释里互相点名"改一处要改两处"）：

```
640 − 16×2 = 608 可用 → (608 − 12×2) / 3 = 194.67 → _entranceCardSizeFor 向下取整 → 194
```

- `chat_page.dart`：`_entranceSheetMaxWidth` / `_entranceSheetHPadding` / 派生 `_entranceCardMaxSize`
- `space_switcher.dart`：同口径的 `_kSheetMaxWidth` / `_kSheetHPadding` / `_kCardMaxSize`
- 手机侧完全不变：480 宽视口下可用宽度 448 → 仍是 141（旧上限 160 本来就够不着），
  只有可用宽度 > 504 的窗口才会看到差别。
- 向下取整仍保留（防浮点误差把第 3 张挤到下一行），代价是右边最多余 2px。

### 验证

- 临时用例实测：两个弹层在 800 / 1600 视口下卡片都是 **160 → 194**，480 视口仍是 141；
  卡片左缘落在内容区（80/480/16 + 16 内边距）上，符合"内容 640 居中"。
- `entrance_list_sheet_test.dart` 补两条：卡片宽 **194**（不是旧的 160）；第一行三张的右缘
