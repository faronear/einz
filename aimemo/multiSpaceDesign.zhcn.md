...
**入口追加（2026-09-24，commit `e1ae4b6`）**：除菜单项外，**状态条**
（`chatPageStatusBar`）里对方名字旁加一个实心下拉三角（`Icons.arrow_drop_down`），
整簇可点 → 一步打开本弹层。动机 = 老板嫌「☰ → 眼扫菜单 → 点『切换我的秘境』」三步啰嗦
（微信是「返回 → 选人」两步）。复用 `chat_page._openSpacePicker`，零路由/零冷启动/零 PIN 改动；
菜单项保留（同一动作两个入口）。详见 `worklog.md` 2026-09-24。
