...
**部署形态（关键前提）：** 系统**只服务于固定的两个人、永远只有一个 Space、不对外分发**——通道由我们亲手安装配置，不存在"邀请陌生人加入"的场景。这一前提让架构可以大幅简化：配对流程整体删除，服务器退化为**静态白名单哑转发器**（见 §2.3、§5、§8）。
...
| 类别   | 功能                                                     |
| ------ | -------------------------------------------------------- |
| 通信   | 文字消息（含输入栏表情符 `[已实现]`）、图片、视频、语音消息（气泡=播放键+固定波形图+秒数 `[已实现]`）、音频文件（气泡=播放键+文件名+h/m/s 时长 `[已实现]`）、已读状态 |
| 空间   | 一次性人工配置（固定两人一空间）、纪念日、在一起的天数   |
| 内容   | 共同回忆、私人笔记                                       |
| 可靠性 | 离线发送、自动同步、Push Notification、消息历史          |
| 安全   | E2EE、服务端加密存储、通道密码学身份、通道撤销、加密备份 |
| 预留   | 一人多通道同步（V1 不实现，见 §2.2）                     |
...
> **2026-09-22 修订**：原表是 v1（单空间 / 静态白名单）的表述，已被 **Multiverse 多租户 +
> 多空间**取代。分层的权威定义与命名见 **`docs/GLOSSARY.md`**（"一台机器 = 一个安装；
> 一个安装 × 一个秘境 = 一个登记项"）。
...
- Server 启动时加载白名单：**不在白名单中的通道一律拒绝**认证、同步、上传——这比任何动态"拒绝第三人"逻辑都更硬。
- 原 `space_members` 表不再需要（§8.3）；Person ≠ Device 建模保留（§2.2），为未来多通道预留。
...
| 威胁                       | 防护                                         |
| -------------------------- | -------------------------------------------- |
| 白名单外通道访问           | 静态白名单：配置之外一律拒绝（§2.3、§8.3）   |
| 手机丢失                   | 通道撤销（白名单）+ 被撤销通道上线自毁（§4、§12）。**自毁只认明确撤销信号**（`entrance.revoked` 帧 / 403 `ENTRANCE_REVOKED`）——服务端库被清空/重置（403 `FORBIDDEN`）或连不上只警告、不清数据（2026-09-16） |
...
E2EE 不等于匿名。V1 承认 Server 可能知道：两个通道在通信、消息时间/大小/数量、附件大小、IP、Device ID、消息类型。**V1 不追求隐藏元数据**。
...
Device Identity Key（长期，X25519 keypair）
    │  私钥永远留在本机 Keychain / Keystore，永不离开通道
...
- **key_version**：每条密文携带其密钥版本号（线格式的一部分，解密按它选密钥）。当前恒为 1——轮换不做（§4.4）。
...
### 4.4 密钥轮换（**不做**，2026-09-14 决策）

- **状态：`[不做]`。** 完整依据（收益矩阵、成本、替代方案、若恢复的前置条件）见
  `docs/SECURITY.md` §3，那是本议题的唯一权威。
- **一句话理由：** 轮换唯一独占的能力是"让已泄露的密钥对**未来**消息失效"；其余动作替代不了它，
  但撤销（掐密文通道）、通道上线自毁、安全存储隔离各自覆盖了更大的风险面；而轮换的成本集中在
  **分发**（新密钥要在不给被撤销通道的前提下送到剩余通道），任一环失败 = 消息永久不可解。
- **撤销的效力来自白名单**（`/sync` 要求白名单通道 → 被撤销通道取不到新密文）+ 被撤销通道
  **上线自毁**（App `_onEntranceRevoked`），不依赖轮换。架构上"取密钥"比"取密文"容易
  （密保箱取包免通道认证，密文必须持白名单 token），所以撤销优先级更高。
...
- **预留：服务器托管加密恢复（模型 B，Signal PIN 模式）。**
  - 恢复密钥用用户口令加密后存 Server，新通道 + 口令即可恢复。
...
- 现在通道由我们亲手安装、公钥由我们亲手登记，**信任在配置现场建立**，不需要协议级握手。
- 服务器退化为"只认两张白名单的哑转发器"（§8）。

### 5.3 配置产物

- Server：`config.json`（两台设备的 entrance_id + public_key + 所属 Person，§8.3）。
- 客户端：导入自己的密封 Space Key；身份私钥不出本机。
- 配置只做一次；未来增加通道（预留的多通道能力）时，用已有可信通道作保（§16）。
...
- 附件访问必须校验：通道已认证 + 在白名单内（§2.3）。
- 引用 / 长按菜单里的预览：图片与视频都显示**缩略图**（不是文件名）——视频取本地首帧
  （`video_thumbnail`，仅 Android/iOS 原生支持，其余平台退回摄像机占位图标）并叠一个播放
  小三角；输入栏引用条为「缩略图 + 文件名」，气泡内引用块只显示缩略图（与图片一致）
  `[已实现]`（老板要求 2026-09-15）。
...
- 生成 Device Identity Key；私钥只存 Keychain / Keystore，**永不离开通道**。
- 一次性配置：生成/导入密钥材料、登记通道公钥、导入密封 Space Key（§5）。
...
- 充当"第二条通道"，端到端验证：一次性配置、E2EE、收发消息、离线队列、增量同步、媒体上传、撤销。
...
- ⚠️ **以下"数据模型要点"是 v1（静态白名单 / config.json）时期的描述，已被 Multiverse（v2）取代**：
  通道登记改为 create/join 动态写入 `entrances` 表，空间归属走 `space_members`，配置只留
...
- **两条硬规则：** ① Server 先持久化消息，再广播/推送；② 任何消息、附件读取都先校验"通道已认证 + 在白名单内"。
...
- 每个 Space 维护单调递增的 `server_sequence`；**不使用时间戳**作为同步顺序（通道时间可能不准、不一致、可被修改）。
- 客户端记录 `last_server_sequence`；重连后 `GET /sync?after=<seq>` 增量拉取，写入本地 SQLite。

### 9.3 实时通道

- WebSocket 用于实时消息、同步事件、投递状态；REST 用于认证、初次同步、附件上传下载、通道管理、备份（一次性配置在安装阶段完成，不走运行时 API）。
...
- 通道安装/登录时注册 Push Token（`push_tokens` 表），撤销通道时移除 Token。
...
- 定期备份 `einz.sqlite.db`（使用 SQLite 官方 Backup API，**禁止直接复制正在写入的 db 文件**）+ 加密文件目录 + 元数据，产物落入 `/data/backups/`。
- 恢复流程必须经过演练并写入运维文档。

### 11.3 客户端备份 / 换机恢复（V1：纯本地，模型 A）

- App 内可导出**加密备份**，密钥由用户**离线保存的恢复码**派生（助记词 / 打印二维码）。
- 新通道：安装 → 输入恢复码 → 解密导入备份 → 重新生成通道凭证并登记入白名单（§5 配置流程）。
- 责任边界：恢复码是"两台设备同时丢失"时的最后保险，须明确提示用户妥善保存；**Server 不接触恢复材料**。

### 11.4 预留（模型 B，Signal PIN 模式）

未来可让恢复密钥用用户口令加密后存 Server，新通道 + 口令即可恢复；V1 不实现，架构不阻塞（§4.5）。

---

## 12. 通道管理

- 客户端可查看通道列表（最后活跃时间），可**撤销**通道。
- 撤销语义：被撤销通道无法再认证、同步、发送。
- **撤销 = 白名单移除 + 清 Push Token/会话 + WS 断开**；被撤销通道**上线即自毁**本地数据。
  **不触发密钥轮换**（§4.4：轮换不做）——它读不到之后的新消息，靠的是白名单而非换钥匙。
- 撤销通道同时清除其 Push Token。
- **历史密文无法追回**（已接受风险）：被撤销通道已同步的密文+旧密钥永远可解（§4.4、`SECURITY.md` §6）。
...
**明确不做（V1 非目标）：** 好友/关注/群聊/公开主页/搜索/Feed/评论点赞；支付/订阅/广告；App Store / Google Play 上架；音视频通话；一人多通道同步；动态配对/邀请（无此场景）；Web 客户端（破坏 E2EE 信任模型）；电脑端正式客户端（暂缓，V2 再评估）。

### 14.2 V2 候选（按需评估）

共享相册、日历、富笔记、位置共享、消息表情回应、贴纸、主题、一人多通道、通道间密钥迁移、搜索、语音/视频通话（WebRTC + STUN/TURN，成熟方案，不自研；**2026-09-18 已出专题方案 `aimemo/voiceCall.zhcn.md`，[待评审]**）、正式电脑端（CLI 加 TUI 升级，或 Flutter 桌面端）。**明确不采用 Web 客户端**（浏览器代码由服务器下发，E2EE 降级）。

### 14.3 开发阶段（估算）

| 阶段    | 内容                                                                                              | 估算    |
| ------- | ------------------------------------------------------------------------------------------------- | ------- |
| Phase 0 | 架构 + 密码学 PoC（`shared/` 核心包 + CLI 测试端，通道密钥、一次性配置、Space Key、加解密、认证） | 2–5 天  |
...
| Phase 4 | 加固（撤销、备份恢复、安全/离线/重启测试；轮换不做，口径见 SECURITY.md）                           | 3–7 天  |
...
```text
docs/E2EE.md      ← 密钥层级、派生、一次性配置的密钥分发、密钥版本、恢复细节
SECURITY.md       ← 安全口径唯一权威：威胁模型、现有控制、不做什么（含轮换）、事件处置手册
...
- [ ] 一人多通道时，新通道加入的"可信通道作保"流程（V2 前再定，§5.3）。
- [ ] 音视频通话的 WebRTC 方案选型 → **已出专题方案（2026-09-18）：`aimemo/voiceCall.zhcn.md`，状态 [待评审]**；边界已定：只做前台语音通话、不做后台呼入、不做视频、不做桌面端。
