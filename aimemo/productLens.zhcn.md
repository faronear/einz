...
- 轮换与撤销的关系（2026-09-14 措辞修正）：**服务器白名单拦截是撤销的第一道防线**（运行时保障，依赖服务器始终正确拦截）；**轮换是密钥侧的第二道防线**——防的是"曾持密钥者 + 事后经其他渠道获得密文"的组合（服务器被攻破/备份泄露/对方设备沦陷时，拦截体系已不在场，唯一防御就是密钥已换）。**轮换不能追溯历史**：被撤销设备已同步的密文+旧密钥永远可解，这部分属设备信任模型已接受的风险（§3.1、DATABASE.md §3）。
- `[待评审]` **密保箱纳入归档密钥**（2026-09-14 提案，详见 `aimemo/escrowArchivedKeys.md`）：
  现状密保箱只含当前 Space Key，轮换过密钥后新设备凭口令接入拿不到归档密钥 → 旧消息
  永久解不开。方案 = payload v2 增加可选 `archived_space_keys`（Server 零改动、向后兼容、
  解锁重传自愈），三端（shared/App/CLI+TUI）一起落地。另：轮换入口（App 菜单/TUI/Server
  端点）目前全未实现，作为独立后续任务待定。
...
- **撤销必须触发 Space Key 轮换**（§4.4，密钥侧第二道防线），否则被撤销设备仍持有旧 Space Key。
- 撤销设备同时清除其 Push Token。
- **历史密文不受轮换影响**（已接受风险）：被撤销设备已同步的密文+旧密钥永远可解（§4.4）。
