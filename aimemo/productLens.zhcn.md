...
> 命名已随 `docs/GLOSSARY.md` 细化：本文里的 "Person" 现在叫**伴侣身份（member）**，
> "Device" 拆成**登记项（entry）**与**安装（install）**两层。分离的**原则不变**，只是层更细。
...
- 充当"第二台设备"，端到端验证：一次性配置、E2EE、收发消息、离线队列、增量同步、媒体上传、撤销。
...
- 新设备：安装 → 输入恢复码 → 解密导入备份 → 重新生成设备凭证并登记入白名单（§5 配置流程）。
...
- **撤销 = 白名单移除 + 清 Push Token/会话 + WS 断开**；被撤销设备**上线即自毁**本地数据。
  **不触发密钥轮换**（§4.4：轮换不做）——它读不到之后的新消息，靠的是白名单而非换钥匙。
- 撤销设备同时清除其 Push Token。
- **历史密文无法追回**（已接受风险）：被撤销设备已同步的密文+旧密钥永远可解（§4.4、`SECURITY.md` §6）。
...
共享相册、日历、富笔记、位置共享、消息表情回应、贴纸、主题、一人多设备、设备间密钥迁移、搜索、语音/视频通话（WebRTC + STUN/TURN，成熟方案，不自研；**2026-09-18 已出专题方案 `aimemo/voiceCall.zhcn.md`，[待评审]**）、正式电脑端（CLI 加 TUI 升级，或 Flutter 桌面端）。**明确不采用 Web 客户端**（浏览器代码由服务器下发，E2EE 降级）。
